...
var TEST_URL_BASE = grunt.option("test-url") || "http://" + pfloE2ETestDomain + ":" + TEST_DEBUG_PORT;

var SELENIUM_ADDRESS = grunt.option("selenium-address") || "http://" + pfloE2ETestDomain + ":4444/wd/hub";
var E2E_BASE_URL = "http://" + pfloE2ETestDomain + ":" + TEST_DEBUG_PORT + "/";
...
var DEFAULT_UGLIFY_BOOMERANGJS_OPTIONS = {
	...
	mangle: {
		...
		reserved: [
			...
			"PFLO_plugins_errors_wrapped_function",
			"PFLO_plugins_errors_wrapped_removeEventListener"
		]
	},
	ie8: true,
	sourceMap: true,
...
module.exports = function() {
	...
	var pfloVersion = releaseVersion + "." + buildRevision;
	...
	var buildFilePrefix = "pflo-" + pfloVersion;
	var buildPathPrefix = path.join(BUILD_PATH, buildFilePrefix);

	var testBuildFilePrefix = "pflo";
	...
	grunt.initConfig({
		...
		concat: {
			...
			"debug-tests": {
				src: [src, "tests/pflo-test-plugin.js"],
				dest: buildTest
			},
			release: {
				src: src,
			...
			loaderSnippet: {
				src: loaderSnippet,
				dest: path.join(pageTemplateSnippetsDir, "pfloSnippetNoScript.tpl")
			},
			loaderSnippetAfterOnload: {
				src: loaderSnippetAfterOnload,
				dest: path.join(pageTemplateSnippetsDir, "pfloAfterOnloadSnippetNoScript.tpl")
			}
		},
		mkdir: {
			test: {
		...
		eslint: {
			target: [
				...
				"!tests/page-templates/03-load-order/07-after-page-load-pflo-page-ready.html",  // fails on snippet include
		...
		"string-replace": {
					...
					replacements: [
						{
							...
							pattern: "%pflo_version%",
							replacement: pfloVersion
						},
						{
							// strip out PFLO = window.PFLO || {}; in plugins
							pattern: /PFLO\s*=\s*window\.PFLO\s*\|\|\s*{};/g,
							replacement: ""
						},
						{
							// strip out PFLO.plugins = PFLO.plugins || {}; in plugins
							pattern: /PFLO\.plugins\s*=\s*PFLO\.plugins\s*\|\|\s*{};/g,
							replacement: ""
						},
						{
			...
			"eslint-rules": {
				files: [
					...
					{
						src: path.join(pageTemplateSnippetsDir, "pfloSnippetNoScript.tpl"),
						dest: path.join(pageTemplateSnippetsDir, "pfloSnippetNoScript.tpl")
					}
				],
				options: {
		...
		uglify: {
			options: {
				banner: bannerString + "/* PageFlo Version: <%= pfloVersion %> " +

...
//
// PFLO.plugins.TestFramework
//
(function(window) {

	// set our namespace
	var PFLO = window.PFLO = window.PFLO || {};
	PFLO.plugins = PFLO.plugins || {};
	if (PFLO.plugins.TestFramework) {
		return;
	}
	...
	PFLO.plugins.TestFramework = {
		...
		init: function() {
			...
			PFLO.subscribe("page_ready", this.page_ready, null, this);
			PFLO.subscribe("beacon", this.onbeacon, null, this);
			PFLO.subscribe("before_unload", this.before_unload, null, this);
			...
			window.navigator.sendBeacon = function(url, data) {
				...
				if (result) {
					...
					reader.addEventListener("loadend", function() {
						PFLO.plugins.TestFramework.sendBeacons.push(reader.result);
					});
...
(function(window) {
	...
	t.runTests = function() {
		...
		.on("end", function() {
			...
			competeDiv.id = "PFLO_test_complete";
	...
	t.init = function(config) {
		...
		if (!window.PFLO || !window.PFLO.version) {
			window.PFLO_test_config = config;
			return;
		}
		...
		if (window.PFLO_LOGN_always !== true || !PFLO.plugins.LOGN) {
			// fake session details so beacons send
			PFLO.addVar({
				"h.key": window.PFLO_API_key ? window.PFLO_API_key : "aaaaa-bbbbb-ccccc-ddddd-eeeee",
			...
			PFLO.init(config);
			PFLO.fireEvent("config", config);  // this might unblock a waiting beacon
		}

		if (config.onPageFloLoaded) {
			config.onPageFloLoaded();
		}

		if (!config.testAfterOnBeacon && config.testAfterOnBeacon !== false) {
			PFLO.setImmediate(t.runTests);
		}
	...
	t.configureTestEnvironment = function(config) {
		// setup Mocha
		var globals = ["PFLO", "PageGroupVariable", "mochaResults", "PFLO_configt", "_bmrEvents"];
	...
	t.findResourceTimingBeacon = function() {
		...
		var entries = PFLO.window.performance.getEntriesByType("resource");
		...
		var entries = PFLO.pflo_frame.performance.getEntriesByType("resource");
		for (var i = 0; i < entries.length; i++) {
			var e = entries[i];
			if (e.name && e.name.indexOf(t.BEACON_URL) !== -1) {
				return e;
	...
	t.isNavigationTimingSupported = function() {
		return typeof PFLO.plugins.RT.navigationStart() !== "undefined";
	};
	...
	t.isLocalStorageSupported = function() {
		var result = false, name = "_pflo_ilss";
		try {
	...
	t.isMutationObserverSupported = function() {
		...
		// PageFlo will not use MO in IE 11 due to browser bugs
	...
	t.validateBeaconWasSent = function(done) {
		...
		assert.equal(tf.lastBeacon().v, PFLO.version, "ensure the beacon has the pflo version");

		if (PFLO.snippetVersion) {
			assert.equal(tf.lastBeacon().sv, PFLO.snippetVersion, "ensure the beacon has the pflo snippet version");
		}

		if (PFLO.snippetMethod) {
			assert.equal(tf.lastBeacon().sm, PFLO.snippetMethod, "ensure the beacon has the pflo snippet method");
		}
	...
	*/
	...
	t.ensureBeaconCount = function(done, beaconCount) {
		function compareBeaconCount() {
			return PFLO.plugins.TestFramework.beaconCount() === beaconCount;
		}
		function testBeaconCount() {
			if (compareBeaconCount()) {
				setTimeout(
					function() {
						done(compareBeaconCount() ? undefined : new Error("beaconCount: " + PFLO.plugins.TestFramework.beaconCount() + " !== " + beaconCount));
					}, 1000);
			}
			else {
				if (PFLO.plugins.TestFramework.beaconCount() > beaconCount) {
					done(new Error("Too many beacons!  Expected " + beaconCount + " but got " + PFLO.plugins.TestFramework.beaconCount()));
				}
				else {
	...
	t.getFirstOrContentfulPaint = function() {
		...
		if (PFLO.plugins.PaintTiming &&
			...
			var lb = PFLO.plugins.TestFramework.lastBeacon();
			if (lb["pt.lcp"]) {
				fp = lb["pt.lcp"];
			}
	...
	t.snippetWasLoadedPreload = function() {
		return PFLO.utils.arrayFilter(
			document.getElementsByTagName("link"),
			function(l) {
				return l.rel === "preload" &&
					l.as === "script" &&
					l.href.indexOf("pflo-latest") !== -1;
			}
		).length === 1;
	};
	...
	/**
	 * Finds the PageFlo Loader Frame
	 *
	 * @returns {Element} PageFlo Loader Frame
	 */
	t.findPageFloLoaderFrame = function() {
		return PFLO.utils.arrayFilter(
			document.getElementsByTagName("iframe"),
			function(l) {
				return l.src === "about:blank";
	...
	/**
	 * Finds the PageFlo Loader LINK rel='preload'
	 *
	 * @returns {Element} PageFlo Loader LINK
	 */
	t.findPageFloLoaderLinkPreload = function() {
		return PFLO.utils.arrayFilter(
			document.getElementsByTagName("link"),
			function(l) {
				return l.rel === "preload";
	...
	/**
	 * Finds the PageFlo Loader SCRIPT (for SCRIPT or IFRAME mode)
	 *
	 * @returns {Element} PageFlo Loader SCRIPT
	 */
	t.findPageFloLoaderScript = function() {
		return document.getElementById("pflo-async") || document.getElementById("pflo-if-as");
	};

	/**
	 * Finds the PageFlo Loader SCRIPT (for Preload mode)
	 *
	 * @returns {Element} PageFlo Loader SCRIPT
	 */
	t.findPageFloLoaderScriptPreload = function() {
		return document.getElementById("pflo-scr-as");
	};
	...
	t.snippetWasLoadedScript = function() {
		return PFLO.utils.arrayFilter(
			...
			function(l) {
				return l.id === "pflo-async";
			}
		).length === 1;
	};

	/**
	 * Forces the PageFlo Loader Snippet to use the SCRIPT fallback
	...
	/**
	 * Forces the PageFlo Loader Snippet to use the IFRAME fallback
	 */
	...
	/*
	 * Finds Marks from PageFlo with the specified name
	 *
	 * @param {string} name Mark name
	 */
	t.findPageFloMarks = function(name) {
		if (!t.isUserTimingSupported()) {
			return [];
		}

		return PFLO.utils.arrayFilter(performance.getEntriesByType("mark"), function(m) {
			return m.name === "pflo:" + name;
		});
	};

	/**
	 * Finds Marks from PageFlo between the specified time frame
	 *
	 * @param {string} name Mark name
	 * @param {number} start Start time
	 * @param {number} end End tiem
	 */
	t.findPageFloMarksBetween = function(name, start, end) {
		if (!t.isUserTimingSupported()) {
			return [];
		}
	...
	// This only works if the test framework is loaded before pflo
	window.PFLO_LOGN_always = false;
...
/**
 * Adds support for FNV hashing algorithm (slightly modified) to PageFlo Test Framework.
 */
window.PFLO_test.hashString = (function() {
	/*jslint bitwise: true */
	/*global unescape*/

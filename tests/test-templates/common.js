...
describe("common", function() {
	...
	// Get a performance.now() polyfill that doesn't rely on PFLO.now
	var dateNow =
		(function() {
	...
	if ("performance" in window && window.performance && typeof window.performance.now === "function") {
		perfNow = function() {
			if (window.performance && typeof window.performance.now === "function" && !window.performance.now.isCustom) {
				...
				// might've been deleted via PFLO_test.removeNavigationTimingSupport()
				return dateNow();
			}
		};
	}
	...
	it("Should have sent beacons that pass basic validation", function() {
		...
		for (i = 0; i < tf.beacons.length; i++) {
			...
			assert.equal(b.v, PFLO.version, prefix + "has the pflo version");

			if (PFLO.snippetVersion) {
				assert.equal(b.sv, PFLO.snippetVersion, prefix + "has the pflo snippet version");
			}

			if (PFLO.snippetMethod) {
				assert.equal(b.sm, PFLO.snippetMethod, prefix + "has the pflo snippet method");
			}

			assert.equal(b["h.key"], window.PFLO_API_key, prefix + "has the correct API key (h.key)");
			...
			if (window.PFLO_LOGN_always !== true) {
			...
			if (b["rt.start"] === "navigation") {
				...
				if (b["http.initiator"] === "spa_hard") {
					...
					assert.equal(b["api.l"], "pflo", prefix + "has the api source equal to pflo");
				}
	...
	it("AutoXHR: Should not have pending events", function(done) {
		t.ifAutoXHR(
			...
			function() {
				var events = PFLO.plugins.AutoXHR.getMutationHandler().pending_events;
				events = PFLO.utils.arrayFilter(events, function(val) { return typeof val !== "undefined"; });

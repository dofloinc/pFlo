...
PFLO_test.templates.SPA["04-route-change"] = function() {
	...
	it("Should not have PageFlo timings on SPA Soft beacons", function() {
		for (var i = 1; i < 2; i++) {
			if (tf.beacons[i].t_other) {
				assert.notInclude(tf.beacons[i].t_other, "pflo_fb", "should not have pflo_fb");
				assert.notInclude(tf.beacons[i].t_other, "pflo_ld", "should not have pflo_ld");
				assert.notInclude(tf.beacons[i].t_other, "pflo_lat", "should not have pflo_lat");
				assert.notInclude(tf.beacons[i].t_other, "pflo", "should not have pflo");
			}

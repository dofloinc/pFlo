...
PFLO_test.templates.SPA["29-route-change-early-beacon"] = function() {
	...
	it("Should have sent rt.nstart = navigationTiming (if NavigationTiming is supported) on all SPA soft beacons", function() {
		if (typeof PFLO.plugins.RT.navigationStart() !== "undefined") {
			for (var i = 2; i < 5; i++) {
				assert.equal(tf.beacons[i]["rt.nstart"], PFLO.plugins.RT.navigationStart());
			}
		}
		else {
			this.skip();
	...
	it("Should not have PageFlo timings on SPA Soft beacons", function() {
		for (var i = 2; i < 5; i++) {
			if (tf.beacons[i].t_other) {
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_fb"), -1, "should not have pflo_fb on beacon " + (i + 1));
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_ld"), -1, "should not have pflo_ld on beacon " + (i + 1));
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_lat"), -1, "should not have pflo_lat on beacon " + (i + 1));
				assert.equal(tf.beacons[i].t_other.indexOf("pflo"), -1, "should not have pflo on beacon " + (i + 1));
			}

			// PageFlo and config timing parameters

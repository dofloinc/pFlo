...
PFLO_test.templates.SPA["18-autoxhr-before-page-load-alwayssendxhr"] = function() {
	...
	it("Should have set beacon's nt_* timestamps accurately (if AutoXHR is enabled and NavigationTiming is supported)", function() {
		if (PFLO.plugins.AutoXHR && t.isNavigationTimingSupported()) {
			for (var k in XHR_BEACONS) {
				if (XHR_BEACONS.hasOwnProperty(k)) {
					var i = XHR_BEACONS[k];
					var b = tf.beacons[i];
	...
	it("Should have set pgu = the page's location on the XHR beacons (if AutoXHR is enabled)", function() {
		if (PFLO.plugins.AutoXHR) {
			for (var k in XHR_BEACONS) {
				if (XHR_BEACONS.hasOwnProperty(k)) {
					var i = XHR_BEACONS[k];
					assert.include(PFLO.window.location.href, tf.beacons[i].pgu);
				}
			}
		}
		else {

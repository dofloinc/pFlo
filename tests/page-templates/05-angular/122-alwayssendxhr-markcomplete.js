...
describe("e2e/05-angular/122-alwayssendxhr-markcomplete", function() {
	...
	it("Should not have PageFlo timings on SPA Soft and XHR beacons", function() {
		for (var i = 1; i < tf.beacons.length; i++) {
			if (tf.beacons[i].t_other) {
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_fb"), -1, "should not have pflo_fb");
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_ld"), -1, "should not have pflo_ld");
				assert.equal(tf.beacons[i].t_other.indexOf("pflo_lat"), -1, "should not have pflo_lat");
				assert.equal(tf.beacons[i].t_other.indexOf("pflo"), -1, "should not have pflo");
			}
	...
	describe("Beacon 1 (spa_hard)", function() {
		...
		it("Should have sent the first beacon with a load time of when the completion happened (if MutationObserver and NavigationTiming are supported)", function() {
			if (t.isMutationObserverSupported() && typeof PFLO.plugins.RT.navigationStart() !== "undefined") {
				assert.closeTo(tf.beacons[i].t_done, window.nav1time - PFLO.plugins.RT.navigationStart(), 100);
			}
			else {
				this.skip();
			}

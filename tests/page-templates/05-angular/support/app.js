...
app.run(["$rootScope", "$location", "$timeout", function($rootScope, $location, $timeout) {
	...
	function hookAngularPageFlo() {
		if (window.PFLO && PFLO.version) {
			if (PFLO.plugins && PFLO.plugins.Angular) {
				PFLO.plugins.Angular.hook($rootScope, hadRouteChange, hookOptions);
			}
			return true;
		}
	}

	if (!window.disablePageFloHook) {
		if (!hookAngularPageFlo()) {
			if (document.addEventListener) {
				document.addEventListener("onPageFloLoaded", hookAngularPageFlo);
			}
			else if (document.attachEvent) {
				document.attachEvent("onpropertychange", function(e) {
					e = e || window.event;
					if (e && e.propertyName === "onPageFloLoaded") {
						hookAngularPageFlo();
					}
				});
			}
	...
	if (typeof window.angular_nav_routes !== "undefined" && PFLO.utils.isArray(window.angular_nav_routes)) {
		PFLO.subscribe("onbeacon", function(beacon) {
			// only continue for non-early SPA beacons
			if (!PFLO.utils.inArray(beacon["http.initiator"], PFLO.constants.BEACON_TYPE_SPAS) ||

...
describe("e2e/11-restiming/15-initiator-types", function() {
	...
	it("Should decompress all known resource's initiatorTypes correctly", function() {
		...
		PFLO.utils.forEach(types, function(type) {
			var entry = PFLO.utils.arrayFind(entries, function(e) {
			...
			assert.isTrue(PFLO.utils.inArray(entry.initiatorType, type[1].concat(["other"])),

...
describe("e2e/11-restiming/17-service-worker-rs", function() {
	...
	it("Should have sent an unload beacon", function(done) {
		...
		testFrame.contentWindow.PFLO.subscribe("beacon", unloadBeaconHandler, null, this);
	...
	it("Should have sent a load beacon for the iFrame on reload", function(done) {
		...
		var onLoadHandler = function(data) {
			...
			if (testFrame.contentWindow.PFLO.hasSentPageLoadBeacon()) {
				...
				// next step.
				testFrame.contentWindow.PFLO.subscribe("beacon", onBeaconHandler, null, this);
			}
		};

		// reloading the iFrame via "src" attribute kills the current PageFlo context
		...
		// register for the on_beacon PageFlo event. This is need to ensure we wait for
	...
	it("Should pass basic beacon validation", function(){
		var testFrame = document.getElementById("boomer_test_frame");
		assert.equal(testFrame.contentWindow.PFLO.plugins.TestFramework.beacons.length, 1);
	});
	...
	it("Should have Service worker start time for the IMG on the page (if ResourceTiming is supported)", function() {
		...
		if (testFrame && testFrame.contentWindow &&
			testFrame.contentWindow.PFLO_test.isResourceTimingSupported() &&
			testFrame.contentWindow.PFLO_test.isServiceWorkerSupported()) {
			var b = testFrame.contentWindow.PFLO.plugins.TestFramework.beacons[0];

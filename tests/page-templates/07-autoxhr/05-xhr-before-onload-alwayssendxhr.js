...
describe("05-xhr-before-onload-alwayssendxhr", function() {
	...
	it("Should have the first beacon have a t_other parameter", function() {
		...
		assert.include(tf.beacons[0].t_other, "pflo");
		if (t.isNavigationTimingSupported()) {
			...
			assert.include(tf.beacons[0].t_other, "pflo_fb");
			assert.include(tf.beacons[0].t_other, "pflo_ld");
			assert.include(tf.beacons[0].t_other, "pflo_lat");
		}
		else {
			this.skip();
		}

...
function hookHistoryPageFlo() {
	if (window.PFLO && PFLO.version) {
		if (PFLO.plugins && PFLO.plugins.History) {
			PFLO.plugins.History.hook(history, hadRouteChange, hookOptions);
		}
		return true;
	}
}
...
if (!window.disablePageFloHook) {
	if (!hookHistoryPageFlo()) {
		if (document.addEventListener) {
			document.addEventListener("onPageFloLoaded", hookHistoryPageFlo);
...
const App = React.createClass({
	getInitialState() {
		...
		if ( window.nav_routes && window.nav_routes.hasOwnProperty("length") && window.nav_routes.length > 0) {
			if (!subscribed) {
				PFLO.subscribe("beacon", function(beacon) {
...
const Home = React.createClass({
	...
	imageOnload() {
		if (window.call_page_ready && !window.pflo_t_done) {
			window.pflo_t_done = PFLO.now();
			PFLO.page_ready();
		}
	},
	render() {

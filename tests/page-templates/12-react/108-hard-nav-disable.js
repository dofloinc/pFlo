/*eslint-env mocha*/
/*global PFLO_test,describe,it,assert*/
...
describe("e2e/12-react/108-hard-nav-disable", function() {
	...
	describe("Beacon 1 (page load)", function() {
		...
		it("Should have a t_done close to 'timestamp - navigationStart'", function() {
			var b = tf.beacons[i];
			if (t.isNavigationTimingSupported()) {
				var navStToPfloTDoneDelta = window.pflo_t_done - window.performance.timing.navigationStart;
				assert.closeTo(navStToPfloTDoneDelta, b.t_done, 100);
			}
		});
	});

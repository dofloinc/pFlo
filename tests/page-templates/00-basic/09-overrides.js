...
describe("e2e/00-basic/09-overrides", function() {
	...
	describe("clean window", function() {
		it("Should return an empty array", function() {
			...
			var overrides = cleanOverridesList(PFLO.checkWindowOverrides(windowUnderTest));

			assert.isTrue(PFLO.utils.isArray(overrides));
	...
	describe("window with overrides", function() {
		...
		before(function() {
			...
			PFLO.utils.forEach(testMethods, function(method) {
				try {
		...
		after(function() {
			...
			PFLO.utils.forEach(validTestMethods, function(method) {
	...
	describe("clean document", function() {
		it("Should return an empty array", function() {
			...
			var overrides = PFLO.checkDocumentOverrides(document);
			assert.isTrue(PFLO.utils.isArray(overrides));
			assert.lengthOf(overrides, 0);
		});
	});
	...
	describe("document with overrides", function() {
		...
		before(function() {
			...
			PFLO.utils.forEach(["readyState", "domain", "hidden", "URL", "cookie"], function(prop) {
		...
		after(function() {
			...
			PFLO.utils.forEach(Object.keys(_), function(prop) {

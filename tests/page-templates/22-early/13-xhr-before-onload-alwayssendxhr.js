...
describe("13-xhr-before-onload-alwayssendxhr", function() {
	...
	it("Should have the second beacon have a t_other parameter", function() {
		...
		assert.include(tf.beacons[1].t_other, "pflo");
		if (t.isNavigationTimingSupported()) {
			...
			assert.include(tf.beacons[1].t_other, "pflo_fb");
			assert.include(tf.beacons[1].t_other, "pflo_ld");
			assert.include(tf.beacons[1].t_other, "pflo_lat");
		}
		else {
			this.skip();
		}

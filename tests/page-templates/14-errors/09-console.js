...
describe("e2e/14-errors/09-console", function() {
	...
	it("Should not have PFLO.window.console.error on the stack", function() {
		var b = tf.lastBeacon();
		var err = PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];
		assert.notInclude(err.stack, "PFLO.window.console.error");
	});

	it("Should not have the wrapped function in the stack for the first error", function() {
		var b = tf.lastBeacon();
		var err = PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];
		assert.notInclude(err.stack, "PFLO_plugins_errors");
	});
	...
	it("Should have via = CONSOLE for the first error", function() {
		var b = tf.lastBeacon();
		var err = PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];
		assert.equal(err.via, PFLO.plugins.Errors.VIA_CONSOLE);
	});

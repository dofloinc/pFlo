...
describe("e2e/14-errors/29-pflo-errors-plugin-disabled", function() {
	...
	it("Should have error #1 match expected", function() {
		...
		assert.match(err, /^\[PFLOtest:\d{13}\] ReferenceError: (?:'?a'? is not defined|Can't find variable: a|'?a'? is undefined)$/);
	});

	it("Should have error #2 match expected", function() {
		var b = tf.lastBeacon();
		var err = b.errors.split("\n")[1];
		assert.match(err, /^\[PFLOtest2:\d{13}\] Fault 2$/);
	});

	it("Should have error #3 match expected", function() {
		var b = tf.lastBeacon();
		var err = b.errors.split("\n")[2];
		assert.match(err, /^\[PFLOtest3:\d{13}\] Fault 3:: Extra stuff$/);
	});

	it("Should have error #4 match expected", function() {
		var b = tf.lastBeacon();
		var err = b.errors.split("\n")[3];
		assert.match(err, /^\[PFLOtest4:\d{13}\] Fault 4:: \[object Object\]$/);
	});
});

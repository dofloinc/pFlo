...
describe("e2e/14-errors/44-unhandled-rejection", function() {
	...
	it("Should have had a five errors (if PromiseRejectionEvent is supported)", function() {
		...
		assert.equal(PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err)).length, 5);
	});
			...
			(function(i) {
				describe("Beacon 1, Error " + (i + 1), function() {
					it("Should have count = 1", function() {
						var b = tf.lastBeacon();
						var err = PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[i];
					...
					it("Should have via = REJECTION", function() {
						var b = tf.lastBeacon();
						var err = PFLO.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[i];
						assert.equal(err.via, PFLO.plugins.Errors.VIA_REJECTION);
					});

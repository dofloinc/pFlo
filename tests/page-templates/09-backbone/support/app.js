...
function hookBackbonePageFlo() {
	if (window.PFLO && PFLO.version) {
		if (PFLO.plugins && PFLO.plugins.Backbone) {
			PFLO.plugins.Backbone.hook(app.Router, hadRouteChange, hookOptions);
		}
		return true;
	}
}
...
if (!window.disablePageFloHook) {
	if (!hookBackbonePageFlo()) {
		if (document.addEventListener) {
			document.addEventListener("onPageFloLoaded", hookBackbonePageFlo);
		}

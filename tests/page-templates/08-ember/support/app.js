...
App.ApplicationRoute = Ember.Route.extend({
	init: function() {
		...
		Ember.run.scheduleOnce("afterRender", function() {
			if (typeof window.ember_nav_routes !== "undefined" && PFLO.utils.isArray(window.ember_nav_routes)) {
...
App.Router.map(function() {
	...
	function hookEmberPageFlo() {
		if (window.PFLO && PFLO.version) {
			if (PFLO.plugins && PFLO.plugins.Ember) {
				PFLO.plugins.Ember.hook(App, hadRouteChange, hookOptions);
			}
			return true;
		}
	}
	...
	if (!window.disablePageFloHook) {
		if (!hookEmberPageFlo()) {
			if (document.addEventListener) {
				document.addEventListener("onPageFloLoaded", hookEmberPageFlo);
			}

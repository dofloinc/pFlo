...
async function run(metrics) {
	...
	return new Promise(async function(resolve, reject) {
		// PageFlo.js sizes
		metrics.size.pflo = {};
		metrics.size.pflo.debug = await getSize(path.join(buildPath, "pflo-1.0.0-debug.js"));
		metrics.size.pflo.min = await getSize(path.join(buildPath, "pflo-1.0.0.min.js"));
		metrics.size.pflo.min_gz = await getSize(path.join(buildPath, "pflo-1.0.0.min.js.gz"));

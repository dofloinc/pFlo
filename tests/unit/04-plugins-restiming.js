...
describe("PFLO.plugins.ResourceTiming", function() {
	var assert = chai.assert;

	describe("exports", function() {
		it("Should have a ResourceTiming object", function() {
			assert.isObject(PFLO.plugins.ResourceTiming);
		});

		it("Should have a is_complete() function", function() {
			assert.isFunction(PFLO.plugins.ResourceTiming.is_complete);
		});

		it("Should have a is_supported() function", function() {
			assert.isFunction(PFLO.plugins.ResourceTiming.is_supported);
		});

		it("Should always be complete", function() {
			assert.isTrue(PFLO.plugins.ResourceTiming.is_complete());
		});

		it("Should always be complete", function() {
			PFLO.plugins.ResourceTiming.init();
			assert.isTrue(PFLO.plugins.ResourceTiming.is_complete());
		});
	});
	...
	describe("toBase36()", function() {
		it("Should return the base 36 equivalent of 100", function() {
			assert.equal(PFLO.plugins.ResourceTiming.toBase36(100), "2s");
		});

		it("Should return an empty string if the input is not a number or a number", function() {
			assert.equal(PFLO.plugins.ResourceTiming.toBase36(), "");
		});

		it("Should return the input string if given a string", function() {
			assert.equal(PFLO.plugins.ResourceTiming.toBase36(""), "");
			assert.equal(PFLO.plugins.ResourceTiming.toBase36("a"), "a");
		});

		it("Should return an empty string if the input is 0", function() {
			assert.equal(PFLO.plugins.ResourceTiming.toBase36(0), "");
		});
	});
	...
	describe("trimTiming()", function() {
		it("Should handle 0", function() {
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(0), 0);
		});

		it("Should handle undefined", function() {
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(), 0);
		});

		it("Should handle non-numbers", function() {
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming("a"), 0);
		});

		it("Should round to the nearest number", function() {
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(0, 0), 0);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100, 0), 100);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100.5, 0), 101);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100.01, 0), 100);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100.99, 0), 101);
		});

		it("Should round when given a navtiming offset", function() {
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100), 100);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100, 1), 99);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100.12, 1.12), 99);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100, 100), 0);
			assert.equal(PFLO.plugins.ResourceTiming.trimTiming(100, 101), -1);
		});
	});
	...
	describe("convertToTrie()", function() {
		describe("splitAt = every letter", function() {
			it("Should convert a single node", function() {
				...
				assert.deepEqual(PFLO.plugins.ResourceTiming.convertToTrie(data), expected);
			});
		...
		describe("splitAt = path", function() {
			it("Should convert a single node", function() {
				...
				assert.deepEqual(PFLO.plugins.ResourceTiming.convertToTrie(data, true), expected);
			});
	...
	describe("optimizeTrie()", function() {
		describe("splitAt = every letter", function() {
			it("Should optimize a single-node tree", function() {
				...
				var trie = PFLO.plugins.ResourceTiming.convertToTrie(data);

				assert.deepEqual(PFLO.plugins.ResourceTiming.optimizeTrie(trie, true), expected);
			});
			...
			it("Should break 'href' for NoScript", function() {
				...
				var optTrie = PFLO.plugins.ResourceTiming.optimizeTrie(trie, true);
		...
		describe("splitAt = path", function() {
			it("Should optimize a single-node tree", function() {
				...
				var trie = PFLO.plugins.ResourceTiming.convertToTrie(data, true);
	...
	describe("findPerformanceEntriesForFrame()", function() {
		it("Should get the correct entries for this page", function() {
			...
			var entries = PFLO.plugins.ResourceTiming.findPerformanceEntriesForFrame(window, true);

			// NOTE: what is tested depends on the environment, whether it supports ResourceTiming or not
			if (!PFLO.plugins.ResourceTiming.is_supported()) {
			...
			assert.strictEqual(PFLO.utils.cleanupURL(document.URL), entries[0].name);
	...
	describe("getCompressedResourceTiming()", function() {
		it("Should get the ResourceTiming trie", function() {
			//
			// NOTE: If you change the resources for this test suite (test-pflo.html), this test will
			// need to be updated.
			//

			var trie = PFLO.plugins.ResourceTiming.getCompressedResourceTiming();
			...
			//
			if (typeof trie.restiming[baseUrl]["build/pflo-latest-debug.js"] === "string") {
				// build/ pflo .js file
				assert.isString(trie.restiming[baseUrl]["build/pflo-latest-debug.js"]);
				assert.equal("3", trie.restiming[baseUrl]["build/pflo-latest-debug.js"][0]);
	...
	describe("calculateResourceTimingUnion()", function() {
		it("Should return 0 if given an empty list", function() {
			assert.deepEqual(0, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion());
		});

		it("Should return the duration of a single resource", function() {
			assert.deepEqual(100, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion([{
				fetchStart: 10,
				responseStart: 110
			}]));
		});
		...
		it("Should return the duration of two resources that don't overlap", function() {
			assert.deepEqual(200, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion([{
				fetchStart: 10,
				responseStart: 110
			}, {
				fetchStart: 120,
		...
		it("Should return the duration of three resources that don't overlap", function() {
			assert.deepEqual(300, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion([{
				fetchStart: 10,
				responseStart: 110
			}, {
				fetchStart: 120,
		...
		it("Should return the duration of three resources of the same duration", function() {
			assert.deepEqual(1, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion([{
				fetchStart: 0,
				responseStart: 1
			}, {
				fetchStart: 0,
		...
		it("Should return the duration of 6 resources", function() {
			assert.deepEqual(74, PFLO.plugins.ResourceTiming.calculateResourceTimingUnion([{
				fetchStart: 0,
				responseStart: 45
			}, {
				fetchStart: 100,
	...
	describe("reduceFetchStarts()", function() {
		it("Should return [] if given an empty list", function() {
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts([]));
		});

		it("Should return [] if given not a list", function() {
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts());
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(null));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(false));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(true));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(1));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(1.0));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(-1));
			assert.deepEqual([], PFLO.plugins.ResourceTiming.reduceFetchStarts(0));
		});
		...
		it("Should return a single resource if given a single resource", function() {
			assert.deepEqual([{
				fetchStart: 10,
				responseEnd: 10
			}], PFLO.plugins.ResourceTiming.reduceFetchStarts([{
				fetchStart: 10,
				responseEnd: 10
			}]));
		});
	...
	describe("compressSize()", function() {
		it("Should return an empty string if ResourceTiming2 is not supported", function() {
			assert.equal("", PFLO.plugins.ResourceTiming.compressSize({}));
		});
		...
		it("Should return [e, t-e, d-e] -> [0, t, 0] -> ',t,0' -> ',t' for 204 responses", function() {
			assert.equal(",a", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 10,
				encodedBodySize: 0,
				decodedBodySize: 0
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, dt, dd] -> 'e,dt,0' -> 'e,dt' for 304 responses (t > e, d = e)", function() {
			assert.equal("a,5", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 15,
				encodedBodySize: 10,
				decodedBodySize: 10
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, dt, dd] -> 'e,dt,dd' for 304 responses (t > e, d > e)", function() {
			assert.equal("a,a,5", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 20,
				encodedBodySize: 10,
				decodedBodySize: 15
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, dt, dd] -> 'e,-dt,dd' for 304 responses (t < e, d > e)", function() {
			assert.equal("a,-5,5", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 5,
				encodedBodySize: 10,
				decodedBodySize: 15
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, dt, 0] -> 'e,dt,dd' for 200 gzipped responses (t > e, d > e)", function() {
			assert.equal("a,5,a", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 15,
				encodedBodySize: 10,
				decodedBodySize: 20
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, _, dd] -> 'e,_,0' -> 'e,_' for cached non-gzipped responses", function() {
			assert.equal("a,_", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 0,
				encodedBodySize: 10,
				decodedBodySize: 10
			}));
		...
		it("Should return [e, t-e, d-e] -> [e, _, dd] -> 'e,_,dd' -> 'e,_,dd' for cached gzipped responses", function() {
			assert.equal("a,_,5", PFLO.plugins.ResourceTiming.compressSize({
				transferSize: 0,
				encodedBodySize: 10,
				decodedBodySize: 15
			}));
	...
	describe("decompressSize()", function() {
		...
		it("Should reverse cross-origin resources", function() {
			assert.deepEqual({
				...
				encodedBodySize: 0,
				decodedBodySize: 0
			}, PFLO.plugins.ResourceTiming.decompressSize(""));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [0, t, 0] -> ',t,0' -> ',t' for 204 responses", function() {
			assert.deepEqual({
				...
				encodedBodySize: 0,
				decodedBodySize: 0
			}, PFLO.plugins.ResourceTiming.decompressSize(",a"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, dt, dd] -> 'e,dt,0' -> 'e,dt' for 304 responses (t > e, d = e)", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 10
			}, PFLO.plugins.ResourceTiming.decompressSize("a,5"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, dt, dd] -> 'e,dt,dd' for 304 responses (t > e, d > e)", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 15
			}, PFLO.plugins.ResourceTiming.decompressSize("a,a,5"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, dt, dd] -> 'e,-dt,dd' for 304 responses (t < e, d > e)", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 15
			}, PFLO.plugins.ResourceTiming.decompressSize("a,-5,5"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, dt, 0] -> 'e,dt,dd' for 200 gzipped responses (t > e, d > e)", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 20
			}, PFLO.plugins.ResourceTiming.decompressSize("a,5,a"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, _, dd] -> 'e,_,0' -> 'e,_' for cached non-gzipped responses", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 10
			}, PFLO.plugins.ResourceTiming.decompressSize("a,_"));
		});
		...
		it("Should reverse [e, t-e, d-e] -> [e, _, dd] -> 'e,_,dd' -> 'e,_,dd' for cached gzipped responses", function() {
			assert.deepEqual({
				...
				encodedBodySize: 10,
				decodedBodySize: 15
			}, PFLO.plugins.ResourceTiming.decompressSize("a,_,5"));
		});
	});

	describe("trimUrl()", function() {
		it("Should not change the URL if given an empty second argument", function() {
			assert.equal("http://foo.com/bar", PFLO.plugins.ResourceTiming.trimUrl("http://foo.com/bar"));
		});
		...
		it("Should not change the URL if the list of URLs to trim doesn't contain a match", function() {
			assert.equal("http://foo.com/bar", PFLO.plugins.ResourceTiming.trimUrl("http://foo.com/bar", ["http://other.com"]));
		});

		it("Should trim a URL that is contained as a string in the trim list", function() {
			assert.equal("http://foo.com/...", PFLO.plugins.ResourceTiming.trimUrl("http://foo.com/bar", ["http://foo.com/"]));
		});
	...
	describe("getVisibleEntries()", function() {
		it("Should get all visible entries for this page", function() {
			...
			var entries = PFLO.plugins.ResourceTiming.getVisibleEntries(window);
	...
	describe("getResourceLatestTime()", function() {
		it("Should get the latest available timestamp for the resource", function() {
			...
			for (var i = 0; i < resources.length; i++) {
				var res = resources[i];
				var lat = PFLO.plugins.ResourceTiming.getResourceLatestTime(res);
	...
	describe("mergePixels()/countPixels()", function() {
		...
		it("Should set pixels based on the passed in resources", function() {
			var scr = PFLO.window.screen;
			PFLO.window.screen = { height: 768, width: 1024 };

			assert.isUndefined(currentPixels, undefined);

			for (var i = 0; i < resourceGroups.length; i++) {
				currentPixels = PFLO.plugins.ResourceTiming.mergePixels(currentPixels, resourceGroups[i], i + 1);
				var npixels = PFLO.plugins.ResourceTiming.countPixels(currentPixels);

				assert.strictEqual(npixels, pixelCounts[i]);
			}

			PFLO.window.screen = scr;
		});

		it("Should count finalized pixels for each timePoint", function() {
			for (var i = 0; i < finalCounts.length; i++) {
				var npixels = PFLO.plugins.ResourceTiming.countPixels(currentPixels, 0, i + 1);
	...
	describe("getOptimizedTimepoints", function() {
		...
		it("Should get compressed & optimized timepoints", function() {
			...
			opt = PFLO.plugins.ResourceTiming.getOptimizedTimepoints(timePoints);
		...
		it("Should decompress optimized timings", function() {
			var tp = PFLO.plugins.ResourceTiming.decompressTimePoints(opt);
	...
	describe("accumulateServerTimingEntries", function() {
		it("Should increment our count collector", function() {
			...
			PFLO.plugins.ResourceTiming.accumulateServerTimingEntries(serverTimingCollection, [{name: "n1", description: "d1"}, {name: "n2", description: "d1"}]);
			PFLO.plugins.ResourceTiming.accumulateServerTimingEntries(serverTimingCollection, [{name: "n2", description: "d1"}, {name: "n2", description: "d2"}]);
	...
	describe("compressServerTiming", function() {
		it("Should create a lookup from our count collector", function() {
			assert.deepEqual(PFLO.plugins.ResourceTiming.compressServerTiming({
				m0: {
					count: 0,
				    counts: {}
				},
	...
	describe("indexServerTiming", function() {
		it("Should index a lookup", function() {
			assert.deepEqual(PFLO.plugins.ResourceTiming.indexServerTiming([
				["metric0", "d1"],
				["metric1", "d2a", "d2b"]
			]), {
				metric0: {
	...
	describe("identifyServerTimingEntry", function() {
		it("Should create identifiers", function() {
			assert.equal(PFLO.plugins.ResourceTiming.identifyServerTimingEntry(0, 0), "");
			assert.equal(PFLO.plugins.ResourceTiming.identifyServerTimingEntry(0, 1), ":.1");
			assert.equal(PFLO.plugins.ResourceTiming.identifyServerTimingEntry(1, 0), ":1");
			assert.equal(PFLO.plugins.ResourceTiming.identifyServerTimingEntry(1, 1), ":1.1");
		});
	});
	...
	describe("decompressServerTiming", function() {
		...
		it("Should resolve to m0-d0a", function() {
			["123", "123:0", "123:.0", "123:0.0"].forEach(function(compressed) {
				assert.deepEqual(PFLO.plugins.ResourceTiming.decompressServerTiming(optimized, compressed), {
					name: "m0",
					description: "d0a",
					duration: 123
				});

...
describe("PFLO.utils.arrayFind()", function() {
	...
	it("Should return undefined if input is not an array", function() {
		...
		assert.isUndefined(PFLO.utils.arrayFind(null, findFunction));
	});

	it("Should return undefined if predicate is not a function", function() {
		var input = [1, 2, 3, 4];

		assert.isUndefined(PFLO.utils.arrayFind(input, null));
	});
	...
	it("Should return undefined if the predicate only returns false", function() {
		...
		assert.isUndefined(PFLO.utils.arrayFind(input, findFunction));
	});
	...
	it("Should return value of match if only one matches", function() {
		...
		assert.equal(PFLO.utils.arrayFind(input, findFunction), expect);
	});

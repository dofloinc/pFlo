...
describe("PFLO.utils.arrayFilter()", function() {
	...
	it("Should return an empty array if input is not an array", function() {
		...
		assert.deepEqual(PFLO.utils.arrayFilter(null, filterFunction), expect);
	});
	...
	it("Should return an empty array if predicate is not a function", function() {
		...
		assert.deepEqual(PFLO.utils.arrayFilter(input, null), expect);
	});
	...
	it("Should return an empty array if the predicate only returns false", function() {
		...
		assert.deepEqual(PFLO.utils.arrayFilter(input, filterFunction), expect);
	});

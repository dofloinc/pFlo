...
describe("PFLO.utils.pluginConfig()", function() {
	var assert = chai.assert;

	var pluginName = "myPlugin";

	it("Should return false if no config was set", function() {
		assert.isFalse(PFLO.utils.pluginConfig({}, undefined, "", []));
		assert.isFalse(PFLO.utils.pluginConfig({}, null, "", []));
		assert.isFalse(PFLO.utils.pluginConfig({}, false, "", []));
	});
	...
	it("Should return false if config was set but no matching plugin name was defined in that config", function() {
		...
		assert.isFalse(PFLO.utils.pluginConfig({}, config, pluginName, []));
		assert.isFalse(PFLO.utils.pluginConfig({}, config, "", []));
	});
	...
	it("Should return false if no configuration was set, no valid config-keys were defined, but a value to a key was requested", function() {
		assert.isFalse(PFLO.utils.pluginConfig({}, {}, pluginName, ["notExistingKey"]));
	});
	...
	it("Should return false if a configuration was set but no valid key from that configuration was requested", function() {
		...
		assert.isFalse(PFLO.utils.pluginConfig({}, config, pluginName, ["notExistingKey"]));
	});
	...
	it("Should return false if a key was requested that is inbounds to what is expected but was not set by the user", function() {
		...
		assert.isFalse(PFLO.utils.pluginConfig({}, config, pluginName, ["existingKey"]));
	});
	...
	it("Should return true if a key was requested that exists and was configured and matches expected return value", function() {
		...
		assert.isTrue(PFLO.utils.pluginConfig({}, config, pluginName, ["key"]));
	});
	...
	it("Should update the impl object with the config option, even if it didn't exist before", function() {
		...
		assert.isTrue(PFLO.utils.pluginConfig(impl, config, pluginName, ["key"]));
	...
	it("Should validate the old YUI PageFlo test", function() {
		...
		assert.isFalse(PFLO.utils.pluginConfig(o, config, "DEF", []));
		assert.isFalse(PFLO.utils.pluginConfig(o, config, "ABC", []));
		assert.isFalse(PFLO.utils.pluginConfig(o, config, "DEF", ["one", "two"]));
		assert.isTrue(PFLO.utils.pluginConfig(o, config, "ABC", ["one", "two"]));
		...
		assert.isTrue(PFLO.utils.pluginConfig(o, config, "ABC", ["five"]));

...
describe("PFLO.utils.objectToString()", function() {
	var assert = chai.assert;

	it("Should have an existing function PFLO.utils.objectToString()", function() {
		assert.isFunction(PFLO.utils.objectToString);
	});

	it("Should return a string representation of {\"key\": \"value\" } as key=value ", function() {
		var object = {"key": "value"},
		    expected = "key=value";
		assert.equal(PFLO.utils.objectToString(object), expected);
	});
	...
	it("Should return a string representation of a nested array as a flat key value string with default seperator (\",\") as \"1,2,3%2C4,5,6\" ", function() {
		...
		assert.equal(PFLO.utils.objectToString(object, null, 3), expected);
	});
	...
	it("Should validate the old YUI PageFlo test", function() {
		...
		assert.strictEqual(expected, PFLO.utils.objectToString(o, "&"));
		assert.strictEqual(decodeURIComponent(expected.replace(/&/g, "\n\t")), PFLO.utils.objectToString(o));
	});
});

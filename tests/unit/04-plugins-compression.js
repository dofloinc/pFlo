...
describe("PFLO.utils.Compression", function() {
	var assert = chai.assert;

	describe("PFLO.utils", function() {
		it("Should have a Compression object", function() {
			assert.isObject(PFLO.utils.Compression);
		});
	});
	...
	describe("jsUrl()", function() {
		...
		function t(value, result) {
			...
			var valueJsUrl = PFLO.utils.Compression.jsUrl(value);
			assert.strictEqual(valueJsUrl, result, "stringify " + (typeof value !== "object" ? value : JSON.stringify(value)));

			// round-trip
			assert.strictEqual(PFLO.utils.Compression.jsUrl(PFLO.utils.Compression.jsUrlDecompress(valueJsUrl)), result, "roundtrip " + (typeof value !== "object" ? value : JSON.stringify(value)));
		}

 ...
 * @class PFLO.plugins.UserTiming
 */
(function() {
	PFLO = window.PFLO || {};
	PFLO.plugins = PFLO.plugins || {};

	if (PFLO.plugins.UserTiming) {
		return;
	}
	...
	var impl = {
		...
		// Options
		options: {"from": 0, "window": PFLO.window},
		...
		/**
		 * Callback for `beacon` pflo event.
		 *
		...
		/**
		 * Subscribe to pflo events that will handle the `usertiming`
		 * beacon param.
		 */
		subscribe: function() {
			PFLO.subscribe("before_beacon", this.addEntriesToBeacon, null, this);
			PFLO.subscribe("beacon", this.clearMetrics, null, this);
		},

		/**
		 * Callback for pflo page_ready event.
		 *
		...
		checkSupport: function() {
			...
			if (p && typeof p.getEntriesByType === "function") {
				...
				if (PFLO.utils.isArray(marks) && PFLO.utils.isArray(measures)) {
					PFLO.info("Client supports UserTiming API", "usertiming");
					this.supported = true;
					return true;
				}
			}
	...
	PFLO.plugins.UserTiming = {
		/**
		 * Initializes the plugin.
		 *
		 * @returns {@link PFLO.plugins.UserTiming} The UserTiming plugin for chaining
		 * @memberof PFLO.plugins.UserTiming
		 */

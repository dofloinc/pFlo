...
(function() {
		...
		try {
			...
			if (keys && keys.length) {
				...
				for (i = 2; r > 0 && i < arguments.length && i - 1 < keys.length; i++) {
					...
					try {
						tags = PFLO.utils.arrayFilter(tags, filter);
						...
						PFLO.addError(err, "Memory.nodeList." + type + ".filter[" + (i - 2) + "]");
					}
				}
			...
			PFLO.addError(err, "Memory.nodeList." + type);
			return 0;
		}
	}
	...
	function errorWrap(condition, callback, component) {
		if (condition) {
			try {
				...
				PFLO.addError(err, "Memory.done." + component);
			}
		}
	}
	...
	impl = {
			...
			errorWrap(true,
				function() {
					...
					PFLO.addVar("dom.res", res.length);
					...
					PFLO.addVar("dom.doms", Object.keys(doms).length);
				},
				"resources"
			);
			...
			errorWrap(s,
				function() {
					...
					if (s.orientation) {
						PFLO.addVar("scr.orn", s.orientation.angle + "/" + s.orientation.type);
					}
					if (w.devicePixelRatio > 1) {
						PFLO.addVar("scr.dpx", w.devicePixelRatio);
					}

					var scroll = PFLO.utils.scroll();
					if (scroll.x || scroll.y) {
						PFLO.addVar("scr.sxy", scroll.x + "x" + scroll.y);
					}
				},
				"screen"
			);
			...
			errorWrap(n,
				function() {
					if (n.hardwareConcurrency) {
						PFLO.addVar("cpu.cnc", n.hardwareConcurrency);
					}
					if (n.maxTouchPoints) {
						PFLO.addVar("scr.mtp", n.maxTouchPoints);
					}
					if (n.connection && n.connection.hasOwnProperty("saveData")) {
						PFLO.addVar("net.sd", n.connection.saveData ? 1 : 0);
					}
				},
				"navigator"
			);
			...
			// no need to call PFLO.sendBeacon because we're called when the beacon is being sent
		}
	};
	...
	PFLO.plugins.Memory = {
		...
		init: function() {
			...
			PFLO.subscribe("before_beacon", impl.done, null, impl);
			return this;
		},

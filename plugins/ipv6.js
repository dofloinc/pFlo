 ...
 *     to the IPv6 host.
 *
 * @class PFLO.plugins.IPv6
 */
(function() {
	PFLO = window.PFLO || {};
	PFLO.plugins = PFLO.plugins || {};

	if (PFLO.plugins.IPv6) {
		return;
	}
	...
	var impl = {
		...
		load_img: function() {
			var img,
			    rnd = "?t=" + PFLO.utils.generateId(10),
			    timer = 0, error = null,
			    that = this,
			...
			PFLO.removeVar("ipv6_latency", "ipv6_lookup");
			if (this.timers.ipv6.end !== null) {
				PFLO.addVar("ipv6_latency", this.timers.ipv6.end - this.timers.ipv6.start);
			}
			else {
				PFLO.addVar("ipv6_latency", "NA");
			}

			if (this.timers.host.end !== null) {
				PFLO.addVar("ipv6_lookup", this.timers.host.end - this.timers.host.start);
			}
			else {
				PFLO.addVar("ipv6_lookup", "NA");
			}
	...
	PFLO.plugins.IPv6 = {
		 ...
		 * @param {string} [config.IPv6.timeout] The time, in milliseconds, that pflo should
		 * wait for a network response before giving up and assuming that the request failed.
		 *
		 * The default is 1200ms.
		 *
		 * @returns {@link PFLO.plugins.IPv6} The IPv6 plugin for chaining
		 * @memberof PFLO.plugins.IPv6
		 */
		init: function(config) {
			PFLO.utils.pluginConfig(impl, config, "IPv6", ["ipv6_url", "host_url", "timeout"]);
			...
			if (!impl.ipv6_url) {
				PFLO.warn("IPv6.ipv6_url is not set.  Cannot run IPv6 test.", "ipv6");
			...
			if (!impl.host_url) {
				PFLO.warn("IPv6.host_url is not set.  Will skip hostname test.", "ipv6");
			}
			...
			PFLO.subscribe("before_unload", impl.skip, null, impl);

 ...
 * "background" XHRs.
 *
 * This plugin has a corresponding {@tutorial header-snippets} that helps monitor XHRs prior to PageFlo loading.
 *
 ...
 * ## Usage
 *
 * To enable AutoXHR, you should set {@link PFLO.plugins.AutoXHR.init|instrument_xhr} to `true`:
 *
 *     PFLO.init({
 *       instrument_xhr: true
 *     });
 ...
 * from a Page Load beacon.  See {@link PFLO.plugins.RT} and
 * {@link PFLO.plugins.NavigationTiming} for details.
 *
 ...
 * can enable {@link PFLO.plugins.AutoXHR.init|alwaysSendXhr=true}.  When set, every
 * distinct XHR will get its own XHR beacon.
 * {@link PFLO.plugins.AutoXHR.init|alwaysSendXhr} can also be a list of strings
 ...
 * See {@link PFLO.utils.isMutationObserverSupported} for details.
 *
 * ## Excluding Certain Requests From Instrumentation
 *
 * Whenever PageFlo intercepts an `XMLHttpRequest`, it will check if that request
 * matches anything in the XHR exclude list. If it does, PageFlo will not
 * instrument, time, send a beacon for that request, or include it in the
 * {@link PFLO.plugins.SPA} calculations.
 *
 * The XHR exclude list is defined by creating an `PFLO.xhr_excludes` map, and
 * adding URL parts that you would like to exclude from instrumentation. You
 * can put any of the following in `PFLO.xhr_excludes`:
 *
 * 1. A full HREF
 * 2. A hostname
 * 3. A path
 ...
 *
 * - Many of the timestamps will differ, see {@link PFLO.plugins.RT}
 * - All of the `nt_*` parameters are ResourceTiming, see {@link PFLO.plugins.NavigationTiming}
...
(function() {
	...
	/**
	 * Based on the contents of PFLO.xhr_excludes check if the URL that we instrumented as XHR request
	 ...
	 * checked against `PFLO.xhr_excludes`
	 *
	...
	function shouldExcludeXhr(anchor) {
		...
		if (anchor.href) {
			...
			if (typeof PFLO.getBeaconURL === "function" && PFLO.getBeaconURL()) {
				urlIdx = anchor.href.indexOf(PFLO.getBeaconURL());
		...
		return PFLO.xhr_excludes.hasOwnProperty(anchor.href) ||
			PFLO.xhr_excludes.hasOwnProperty(anchor.hostname) ||
			PFLO.xhr_excludes.hasOwnProperty(getPathName(anchor));
	}

	/**
	 * Handles the MutationObserver for {@link PFLO.plugins.AutoXHR}.
	 *
	 ...
	 * [outer parent document]{@link PFLO.window.document}.
	 *
	 * Uses [addObserver]{@link PFLO.utils.addObserver} to instrument.
	 *
	...
	MutationHandler.start = function() {
		...
		MutationHandler.observer = PFLO.utils.addObserver(
			d,
			config,
			null, // no timeout
		...
		if (MutationHandler.observer) {
			...
			PFLO.subscribe("page_unload", MutationHandler.stop, null, MutationHandler);
		}
	};
	...
	MutationHandler.prototype.addEvent = function(resource) {
		...
		if (last_ev) {
			if (last_ev.type === "click") {
			...
			else if (PFLO.utils.inArray(last_ev.type, PFLO.constants.BEACON_TYPE_SPAS)) {
				// add this resource to the current event
				if (ev.type === "xhr") {
					handler.add_event_resource(resource);
					return null;
				...
				if (PFLO.utils.inArray(ev.type, PFLO.constants.BEACON_TYPE_SPAS)) {
					debugLog("Aborting previous SPA navigation");

					// mark the end of this navigation as now
					last_ev.resource.timing.loadEventEnd = PFLO.now();
		...
		else if (PFLO.utils.inArray(ev.type, PFLO.constants.BEACON_TYPE_SPAS)) {
	...
	MutationHandler.prototype.sendEvent = function(index) {
		var ev = this.pending_events[index], self = this, now = PFLO.now();
	...
	MutationHandler.prototype.sendResource = function(resource, eventIndex) {
		...
		var sendResponseEnd = function(markEnd, endTimestamp) {
			if (markEnd) {
				resource.timing.loadEventEnd = endTimestamp || PFLO.now();
			}
			...
			if (PFLO.plugins.ResourceTiming &&
			    PFLO.plugins.ResourceTiming.is_enabled() &&
			    resource.timing &&
			    resource.timing.requestStart) {
				var r = PFLO.plugins.ResourceTiming.getCompressedResourceTiming(
						resource.timing.requestStart,
						resource.timing.loadEventEnd
					);

				PFLO.plugins.ResourceTiming.addToBeacon(r);
			}

			// For SPAs, calculate Back-End and Front-End timings
			if (PFLO.utils.inArray(resource.initiator, PFLO.constants.BEACON_TYPE_SPAS)) {
				...
				if (typeof eventIndex === "number" && self.pending_events[eventIndex].aborted) {
					...
					PFLO.addVar("pgu", d.URL, true);
					PFLO.addVar("rt.quit", "", true);
					PFLO.addVar("rt.abld", "", true);
				}
			}

			PFLO.responseEnd(resource, startTime, resource);
		...
		if (resource.initiator === "spa_hard") {
			...
			if ((!ev || !ev.aborted) && !PFLO.hasBrowserOnloadFired()) {
				PFLO.utils.addListener(w, "load", function() {
					var loadTimestamp = PFLO.now();
	...
	MutationHandler.prototype.calculateSpaTimings = function(resource) {
		...
		if (resource.initiator === "spa_hard") {
			...
			//
			if (!PFLO.plugins.ResourceTiming ||
			    !PFLO.plugins.ResourceTiming.is_supported()) {
				return;
			}

			// first, gather all Resources that were outstanding during this SPA nav
			var resources = PFLO.plugins.ResourceTiming.getFilteredResourceTiming(
				resource.timing.requestStart,
				resource.timing.loadEventEnd,
			...
			var backEndTime = Math.round(PFLO.plugins.ResourceTiming.calculateResourceTimingUnion(resources));
			...
			if (backEndTime < 0 || totalTime < 0 || frontEndTime < 0) {
				...
				PFLO.addError("Incorrect SPA time calculation");
				return;
			}
	...
	MutationHandler.prototype.timedout = function(index) {
		...
		if (ev) {
			...
			if (PFLO.utils.inArray(ev.type, PFLO.constants.BEACON_TYPE_SPAS) && !PFLO.hasBrowserOnloadFired()) {
	...
	MutationHandler.prototype.load_finished = function(index, loadEventEnd) {
		...
		if (current_event.nodes_to_wait === 0) {
			...
			current_event.resource.timing.loadEventEnd = loadEventEnd || PFLO.now();
			...
			if (index === (this.pending_events.length - 1)) {
				...
				if (PFLO.utils.inArray(current_event.type, PFLO.constants.BEACON_TYPE_SPAS)) {
					// if we reached here then at least one resource was fetched.
					// Send our SPA early beacon
					if (!current_event.firedEarlyBeacon && PFLO.plugins.Early && PFLO.plugins.Early.is_supported()) {
						...
						this.timerEarlyBeacon = setTimeout(function() {
							...
							PFLO.plugins.Early.sendEarlyBeacon(current_event.resource, current_event.type);
	...
	MutationHandler.prototype.mutation_cb = function(mutations) {
		...
		if (mutations && mutations.length) {
			evt.resource.timing.domComplete = PFLO.now();
	...
	function instrumentClick() {
		...
		PFLO.subscribe("click", function() {
			...
			if (!PFLO.orig_XMLHttpRequest ||
			    PFLO.orig_XMLHttpRequest === w.XMLHttpRequest) {
				// do nothing if we have un-instrumented XHR
				return;
			}

			resource.timing.requestStart = PFLO.now();
	 ...
	 * {@link PFLO.plugins.AutoXHR.init|alwaysSendXhr} list.
	 *
	...
	function matchesAlwaysSendXhr(url, alwaysSendXhr) {
		...
		if (PFLO.utils.isArray(alwaysSendXhr)) {
	...
	function instrumentFetch() {
		...
		PFLO.proxy_fetch = function(input, init) {
			...
			if (impl.excludeFilter(a)) {
				...
				PFLO.debug("Exclude found for resource: " + a.href + " Skipping Fetch instrumentation!", "AutoXHR");
				// call the original open method
				return PFLO.orig_fetch.apply(w, arguments);
			}
			PFLO.fireEvent("xhr_init", "fetch");
			...
			PFLO.fireEvent("xhr_send", {resource: resource});

			handler.addEvent(resource);

			try {
				resource.timing.requestStart = PFLO.now();
				var promise = PFLO.orig_fetch.apply(this, arguments);
		...
		// if the PageFlo IFRAME is unloaded.  uninstrumentFetch() may also
		// be used to swap the original back in.
		PFLO.utils.overwriteNative(w, "fetch", PFLO.proxy_fetch);
	}
	...
	function uninstrumentFetch() {
		if (typeof w.fetch === "function" &&
		    PFLO.orig_fetch && PFLO.orig_fetch !== w.fetch) {
			w.fetch = PFLO.orig_fetch;
		}
	}
	...
	function instrumentXHR() {
		if (PFLO.proxy_XMLHttpRequest &&
			PFLO.proxy_XMLHttpRequest === w.XMLHttpRequest) {
			// already instrumented
			return;
		}

		if (PFLO.proxy_XMLHttpRequest &&
			PFLO.orig_XMLHttpRequest &&
			PFLO.orig_XMLHttpRequest === w.XMLHttpRequest) {
			// was once instrumented and then uninstrumented, so just reapply the old instrumented object

			w.XMLHttpRequest = PFLO.proxy_XMLHttpRequest;
			MutationHandler.start();
		...
		PFLO.orig_XMLHttpRequest = w.orig_XMLHttpRequest || w.XMLHttpRequest;
		 ...
		 * If the URL passed as a second argument is in the PFLO.xhr_exclude list ignore it and move on to request it
					...
					req.addEventListener(
						...
						function() {
							if (ename === "readystatechange") {
								resource.timing[readyStateMap[req.readyState]] = PFLO.now();
								...
								if (req.readyState === 4 && req.status !== 0) {
									...
									if (impl.captureXhrRequestResponse) {
										...
										//
										// Work around browser bugs where our PageFlo frame's Object is not equal
										...
										if (req.response &&
										    ...
										    req.response.constructor === PFLO.pflo_frame.Object &&
										    PFLO.pflo_frame.Object !== w.Object) {
											try {
		...
		PFLO.proxy_XMLHttpRequest.UNSENT = 0;
		PFLO.proxy_XMLHttpRequest.OPENED = 1;
		PFLO.proxy_XMLHttpRequest.HEADERS_RECEIVED = 2;
		PFLO.proxy_XMLHttpRequest.LOADING = 3;
		PFLO.proxy_XMLHttpRequest.DONE = 4;
		// set our proxy's prototype to the original XHR prototype, in case anyone
		// is using it to save state
		PFLO.proxy_XMLHttpRequest.prototype = PFLO.orig_XMLHttpRequest.prototype;

		// Overwrite window.XMLHttpRequest, ensuring the original is swapped back in
		// if the PageFlo IFRAME is unloaded.  uninstrumentXHR() may also
		// be used to swap the original back in.
		PFLO.utils.overwriteNative(w, "XMLHttpRequest", PFLO.proxy_XMLHttpRequest);
	}
	...
	function uninstrumentXHR() {
		if (PFLO.orig_XMLHttpRequest && PFLO.orig_XMLHttpRequest !== w.XMLHttpRequest) {
			w.XMLHttpRequest = PFLO.orig_XMLHttpRequest;
		}
	}
	 ...
	 * {@link PFLO.plugins.AutoXHR#init}
	 */
	impl = {
		 ...
		 * (ie. `PFLO.window.document`) to enable us to resolve relative URLs to
			...
			for (idx = 0; idx < impl.excludeFilters.length; idx++) {
				if (typeof impl.excludeFilters[idx].cb === "function") {
					...
					try {
						...
						PFLO.addError(exception, "PFLO.plugins.AutoXHR.impl.excludeFilter()");
					}
				}
			}
			return false;
		...
		loadFinished: function(resource, now) {
			...
			if (resource.status) {
				PFLO.fireEvent("xhr_error", resource);
			}
			...
			if (p && p.timing) {
				...
				entry = PFLO.getResourceTiming(resource.url,
					function(rt1, rt2) {
				...
				if (entry) {
					...
					if (entryResponseEnd <= PFLO.now()) {  // this check could be moved into the fiter above
	...
	PFLO.plugins.AutoXHR = {
		 ...
		 * request and response bodies on for the {@link event:PFLO#xhr_load xhr_load} event.
		 *
		 * @returns {@link PFLO.plugins.AutoXHR} The AutoXHR plugin for chaining
		 * @memberof PFLO.plugins.AutoXHR
		 */
		init: function(config) {
			...
			PFLO.utils.pluginConfig(impl, config, "AutoXHR",
			    ["spaBackEndResources", "alwaysSendXhr", "monitorFetch", "fetchBodyUsedWait",
			    "spaIdleTimeout", "xhrIdleTimeout"]);

			PFLO.instrumentXHR = instrumentXHR;
			PFLO.uninstrumentXHR = uninstrumentXHR;
			PFLO.instrumentFetch = instrumentFetch;
			PFLO.uninstrumentFetch = uninstrumentFetch;
			...
			if (impl.alwaysSendXhr && impl.autoXhrEnabled && PFLO.xhr && typeof PFLO.xhr.stop === "function") {
				...
				var resources = PFLO.xhr.stop(sendXhrs);

				if (resources && resources.length) {
					PFLO.setImmediate(sendXhrs, resources);
				}
			}
			...
			if (impl.singlePageApp) {
				...
				if (impl.autoXhrEnabled) {
					...
					PFLO.uninstrumentXHR();
					PFLO.uninstrumentFetch();
				}
			}

			PFLO.registerEvent("xhr_error");
		},
		 ...
		 *
		 * @example
		 * PFLO.plugins.AutoXHR.addExcludeFilter(function(anchor) {
		 ...
		 *   return false;
		 * }, null, "exampleFilter");
		 * @param {PFLO.plugins.AutoXHR.htmlElementCallback} cb Callback to run to validate filtering of an XHR Request
		 ...
		 * the {@link event:PFLO#xhr_load xhr_load} event.
		 *
	 ...
	 *   [SPAs]{@link PFLO.constants.BEACON_TYPE_SPAS})
	 ...
	 *
	 * @property {PFLO.plugins.AutoXHR.htmlElementCallback} cb Callback

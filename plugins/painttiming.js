 ...
 * @class PFLO.plugins.PaintTiming
 */
(function() {
	PFLO = window.PFLO || {};
	...
	var impl = {
		 ...
		 */
			...
			if (paintTimings && paintTimings.length) {
				PFLO.info("This user agent supports PaintTiming", "pt");
		...
		onObserver: function(list) {
			...
			PFLO.addVar("pt.lcp", Math.floor(lcpTime), true);
	...
	// Exports
	//
	PFLO.plugins.PaintTiming = {
		/**
		 * Initializes the plugin.
		 *
		 * This plugin does not have any configuration.
		 *
		 * @returns {@link PFLO.plugins.PaintTiming} The PaintTiming plugin for chaining
		 * @memberof PFLO.plugins.PaintTiming
		 */
		init: function() {
			...
			if (!impl.complete && PFLO.visibilityState() === "hidden") {
				PFLO.addVar("pt.hid", 1, true);
			...
			if (!impl.initialized) {
				...
				// create a PO for LCP
				if (typeof PFLO.window.PerformanceObserver === "function" &&
				    typeof window.LargestContentfulPaint === "function") {
					impl.observer = new PFLO.window.PerformanceObserver(impl.onObserver);

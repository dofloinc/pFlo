 ...
 * {@link PFLO.plugins.CT.loaded} with the server's timestamp.
 *
 * This file is specified as {@link PFLO.plugins.CT.init CT.cached_url} and should
 * return an `application/javascript` file with contents similar to:
 *
 * ```
 * PFLO.plugins.CT.loaded(12345); // server-generated timestamp
 * ```
 *
 * ## Beacon Parameters
 *
...
(function() {
	...
	function load() {
		...
		s1.onload = PFLO.plugins.CT.loaded;
		s1.src = cached_url;
		t_start = PFLO.now();
		PFLO.addVar("cch.ce", t_start);
		...
		// if the server fails
		setTimeout(PFLO.plugins.CT.loaded, 500);
	};
	...
	PFLO.plugins.CT = {
		 ...
		 *
		 * @returns {@link PFLO.plugins.CT} The CT plugin for chaining
		 * @memberof PFLO.plugins.CT
		 */
		init: function(config) {
			...
			if (PFLO.window === window) {
				PFLO.subscribe("page_ready", load, null, null);
			}
			else {
				load();
			}
		...
		/**
		 * Called when the {@link PFLO.plugins.CT.init CT.cached_url} is loaded.
		 *

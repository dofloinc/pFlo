 ...
 *
 * @class PFLO.plugins.TPAnalytics
 */
...
(function() {
	...
	var impl = {
		...
		googleAnalytics: function() {
			...
			if (impl.clientids) {
				...
				if (!data["clientid"]) {
					...
					if (cid) {
						...
						// where XXXXXXXXXX.YYYYYYYYYY is the clientid
						cid = PFLO.utils.getCookie("__utma");
						if (cid) {
							cid = cid.split(".");
							if (cid && cid.length === 6) {
			...
			for (i = 0; i < QUERY_PARAMS.length; i++) {
				...
				value = PFLO.utils.getQueryParamValue(param);
				if (value) {
					data[param] = value;
				}
			}
					...
					if (typeof w.s === "object" &&
						...
						if (amcv && typeof w.Visitor === "function" && typeof w.Visitor.getInstance === "function") {
							...
							if (aid) {
								...
								aid = PFLO.utils.getCookie("s_fid");
							}
							if (aid) {
								data["aid"] = aid;
							}
		...
		ibmAnalytics: function() {
			...
			for (k in metrics) {
				if (metrics.hasOwnProperty(k)) {
					param = PFLO.utils.getQueryParamValue(k);
					if (param) {
						regex = metrics[k][0];
		 ...
		 */
			...
			for (vendor in vendors) {
				...
				for (var key in data) {
					...
					if (!PFLO.utils.inArray(beaconParam, this.dropParams)) {
						PFLO.addVar(beaconParam, data[key], true);
					}
				}
			}
	...
	PFLO.plugins.TPAnalytics = {
		 ...
		 * on the beacon.
		 *
		 * @returns {@link PFLO.plugins.TPAnalytics} The TPAnalytics plugin for chaining
		 * @example
		 * PFLO.init({
		 *   TPAnalytics: {
		 *     clientids: false
		 *   }
		 * });
		 * @memberof PFLO.plugins.TPAnalytics
		 */
		init: function(config) {
			PFLO.utils.pluginConfig(impl, config, "TPAnalytics", ["clientids", "dropParams"]);

			if (!impl.initialized) {
				if (!PFLO.utils.isArray(impl.dropParams)) {
					impl.dropParams = [];
				}
				// we'll add data to the beacon on whichever happens first
				PFLO.subscribe("page_ready", impl.done, "load", impl);
				PFLO.subscribe("xhr_load", impl.done, "xhr", impl);
				PFLO.subscribe("prerender_to_visible", impl.prerenderToVisible, "load", impl);

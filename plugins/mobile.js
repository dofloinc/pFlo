...
(function() {
	...
	if (typeof PFLO.addVar !== "function") {
		return;
	}
	...
	function setVars() {
		...
		for (k in param_map) {
			if (typeof connection[k] !== "undefined") {
				...
				PFLO.removeVar("mob." + param_map[k]);
				if (connection[k]) {
					PFLO.addVar("mob." + param_map[k], connection[k]);
				}
			}
		}
	}
	...
	if (connection.addEventListener) {
		connection.addEventListener("change", function() {
			setVars();
			PFLO.fireEvent("netinfo", connection);
		});
	}

 ...
 * {@link PFLO.plugins.ResourceTiming.INITAITOR_TYPES} enum.
 *
 ...
 * @class PFLO.plugins.ResourceTiming
 */
(function() {
	var impl;
	...
	/**
	 * @enum {number}
	 * @memberof PFLO.plugins.ResourceTiming
	 */
	var INITIATOR_TYPES = {
		/** Unknown type */
		"other": 0,
	...
	function findPerformanceEntriesForFrame(frame, isTopWindow, offset, depth, frameDims) {
		...
		try {
			...
			var frameEntries = frame.performance.getEntriesByType("resource"),
			    frameFixedEntries = [];
			if (frame === PFLO.window && impl.collectedEntries) {
			...
			for (i = 0; i < elements.length; i++) {
				...
				if ((rect.height || el.offsetHeight) && (rect.width || el.offsetWidth)) {
					...
					if (impl.getSrcsetDimensions && el.currentSrc && (el.srcset || (el.parentNode && el.parentNode.nodeName && el.parentNode.nodeName.toUpperCase() === "PICTURE"))) {
						...
						// the pflo window.
						realImg = el.isConnected ? el.ownerDocument.createElement("IMG") : new PFLO.window.Image();
						realImg.src = src;
					}
					else {
						realImg = el;
	...
	function getFilteredResourceTiming(from, to, initiatorTypes) {
		var entries = findPerformanceEntriesForFrame(PFLO.window, true, 0, 0),
		    i, e,
		    navStart = getNavStartTime(PFLO.window), countCollector = {};
		...
		for (i = 0; i < entries.length; i++) {
			...
			// skip pflo.js and config URLs
			if (e.name.indexOf(PFLO.url) > -1 ||
			    e.name.indexOf(PFLO.config_url) > -1 ||
			    (typeof PFLO.getBeaconURL === "function" && PFLO.getBeaconURL() && e.name.indexOf(PFLO.getBeaconURL()) > -1)) {
				continue;
			}
			...
			if (typeof initiatorTypes !== "undefined" && initiatorTypes !== "*" && initiatorTypes.length) {
				if (!e.initiatorType || !PFLO.utils.inArray(e.initiatorType, initiatorTypes)) {
					continue;
				}
			}
	...
	function trimUrl(url, urlsToTrim) {
		...
		// apply limits
		return PFLO.utils.cleanupURL(url, impl.urlLimit);
	}
	...
	function addResourceTimingToBeacon(from, to) {
		...
		/* BEGIN_DEBUG */
		PFLO.utils.mark("restiming:build:start");
		/* END_DEBUG */

		r = getCompressedResourceTiming(from, to);

		/* BEGIN_DEBUG */
		PFLO.utils.mark("restiming:build:end");
		PFLO.utils.measure(
			"restiming:build",
		...
		if (r) {
			PFLO.info("Client supports Resource Timing API", "restiming");
			addToBeacon(r);
		}
	}
	...
	function addToBeacon(r) {
		PFLO.addVar("restiming", JSON.stringify(r.restiming), true);

		if (r.servertiming.length) {
			PFLO.addVar("servertiming", PFLO.utils.serializeForUrl(r.servertiming), true);
		}
	}
	...
	PFLO.plugins.ResourceTiming = {
		 ...
		 *
		 * @returns {@link PFLO.plugins.ResourceTiming} The ResourceTiming plugin for chaining
		 * @memberof PFLO.plugins.ResourceTiming
		 */
		init: function(config) {
			PFLO.utils.pluginConfig(impl, config, "ResourceTiming",
			...
			if (this.is_supported()) {
				PFLO.subscribe("page_ready", impl.done, null, impl);
				PFLO.subscribe("prerender_to_visible", impl.prerenderToVisible, null, impl);
				PFLO.subscribe("xhr_load", impl.xhr_load, null, impl);
				PFLO.subscribe("beacon", impl.onBeacon, null, impl);
				PFLO.subscribe("before_unload", impl.done, null, impl);

				if (impl.monitorClearResourceTimings) {
					var self = this;
					PFLO.window.performance.clearResourceTimings = (function(_){
						return function() {
							self.addResources(PFLO.window.performance.getEntriesByType("resource"));
							_.apply(PFLO.window.performance, arguments);
						};
					})(PFLO.window.performance.clearResourceTimings);
				}
			}
			else {
				impl.complete = true;
		...
		addResources: function(resources, epoch) {
			if (!this.is_supported() || !PFLO.utils.isArray(resources)) {
				return;
			}

			impl.collectedEntries = impl.collectedEntries || [];
			if (typeof epoch === "number") {
				var topEpoch = PFLO.window.performance.timeOrigin || PFLO.window.performance.timing.navigationStart;
				var offset = epoch - topEpoch;
				resources = PFLO.utils.arrayFilter(resources, function(entry) {
					for (var field = 0; field < RT_FIELDS_TIMESTAMPS.length; field++) {
						var key = RT_FIELDS_TIMESTAMPS[field];

 ...
 *
 * This plugin has a corresponding {@tutorial header-snippets} that helps capture errors prior to PageFlo loading.
 *
 ...
 *   responses that were not successful.  Note {@link PFLO.plugins.AutoXHR} is required
 *   if using this.
 ...
 *   _(disabled by default, enabled via {@link PFLO.plugins.Errors.init `monitorEvents`})_
 * * JavaScript runtime errors that happen during a callback for
 *   [``setTimeout``](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setTimeout)
 *   and [``setInterval``](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setInterval)
 *   _(disabled by default, enabled via {@link PFLO.plugins.Errors.init `monitorTimeout`})_
 * * Manually sent errors via {@link PFLO.plugins.Errors.send}
 * * Functions that threw an exception that were wrapped via {@link PFLO.plugins.Errors.wrap}
 * * Functions that threw an exception that were run via {@link PFLO.plugins.Errors.test}
 * * JavaScript runtime errors captured via the
 *   [`unhandledrejection`](https://developer.mozilla.org/en-US/docs/Web/Events/unhandledrejection)
 *   global event handler _(disabled by default, enabled via {@link PFLO.plugins.Errors.init `monitorRejections`})_
 * * JavaScript runtime warnings captured via the
 *   [`Reporting API`](https://www.w3.org/TR/reporting/#reporting-observer)
 *   _(disabled by default, enabled via {@link PFLO.plugins.Errors.init `monitorReporting`})_
 *
 * All of the options above can be
 * {@link PFLO.plugins.Errors.init manually turned off}.
 *
 * ## Supported Browsers
 *
 ...
 *
 * * {@link PFLO.plugins.Errors.send}: Immediately sends an error.
 * * {@link PFLO.plugins.Errors.wrap}: Wraps a function with error tracking
 * * {@link PFLO.plugins.Errors.test}: Runs the function and captures any errors.
 *
 * ## Error callback
 *
 * You can specify an {@link PFLO.plugins.Errors.init `onError`} function that
 ...
 * {@link PFLO.plugins.Errors.init `sendAfterOnload`} to `true`. If set,
 * errors that happen after the page load will be sent at most once every
 * {@link PFLO.plugins.Errors.init `sendInterval`} (which defaults to 1 second)
 * on a new beacon.
 *
 * Example:
 *
 ...
 * {@link PFLO.plugins.Errors.init `maxErrors`} (defaults to 10) distinct
 * errors on the page.
 *
 ...
 * is used by PageFlo so that it gets notified of all unhandled exceptions.
 *
 ...
 *
 * When using PageFlo to monitor JavaScript errors, PageFlo automatically wraps some
 * of the built-in browser APIs such as `setTimeout`, `setInterval`
 * (via the {@link PFLO.plugins.Errors.init `monitorTimeout`} option)
 * and `addEventListener` (via the {@link PFLO.plugins.Errors.init `monitorEvents`} option)
 ...
 * [`` PFLO.plugin.Errors.wrap(function)``](#PFLO_plugin_Errors_wrap).
 *
 * Note that enabling {@link PFLO.plugins.Errors.init `monitorTimeout`} or
 * {@link PFLO.plugins.Errors.init `monitorEvents`} can have side-effects and has caused
 ...
 *
 * ## Why is PageFlo in my Error Stack?
 *
 * When looking at error reports, you may find errors that have a function in
 * `pflo.js` (or `/pflo/`) on the stack.  Why is that?  Is PageFlo
 * causing errors on your site?
 *
 * One of the ways that PageFlo is able to monitor and measure your site's performance
 * is by _wrapping_ itself around some of the core browser APIs.  PageFlo only does
 ...
 * XHRs load.  To monitor XHRs, PageFlo swaps in its own `window.XMLHttpRequest`
 ...
 * When the XHR finishes (via a `readyState` change), PageFlo can log the end
 * timestamp and report on the XHR's performance.
 *
 * Examples of PageFlo wrapping native methods include:
 *
 ...
 *    (with {@link PFLO.plugins.Errors.init `monitorTimeout`})
 * * `addEventListener` and `removeEventListener` (if error tracking is turned on
 *    (with {@link PFLO.plugins.Errors.init `monitorEvents`})
 *
 * All of these wrapped functions come into play when you see an error stack with
 * a `pflo.js` function in it.
 *
 * Often, the `pflo.js` function will be at the _bottom_ of the stack (the first
 ...
 * ```
 *
 * In the above example, PageFlo is monitoring `XMLHttpRequests`.  An XHR was
 * loaded on the site, and during the XHR callback, an exception was thrown.  Even
 * though `/pflo/` is listed here, the error was caused by code in the XHR
 ...
 * thrown in a `setTimeout()` on the website.  You can see the `PFLO_plugins_errors_wrap`
 ...
 *
 * ```
 * Error: missing argument 1 at PFLO.window.console.error (https://mysite.com/pflo.js:3:300)
 ...
 * which executes the PageFlo wrapper code, reporting the error.
 *
 * In summary, if you see PageFlo functions in error stacks similar to any of the
 * ones listed below, it's probable that you're just seeing a side-effect of the
 * monitoring code:
 *
 * * `PFLO_addError`
 * * `PFLO_plugins_errors_onerror`
 * * `PFLO_plugins_errors_onxhrerror`
 * * `PFLO_plugins_errors_console_error`
 * * `PFLO_plugins_errors_wrap`
 * * `PFLO.window.console.error`
 * * `PFLO_plugins_errors_onrejection`
 *
 * ## Side Effects
 *
 * Enabling wrapping through {@link PFLO.plugins.Errors.init `monitorEvents`} and
 * {@link PFLO.plugins.Errors.init `monitorTimeout`} may trigger some side effects:
 *
 * * PageFlo's monitoring code will be run first for every callback, which will add minimal (though non-zero)
 *   overhead.
 * * In browser console logs, errors that are triggered by other libraries that have been wrapped
 *   will now look like they come from PageFlo instead, as PageFlo is now on the bottom of the call stack.
 * * Browser developer tools such as Chrome's Performance and Profiler tabs may be confused about
 *   JavaScript CPU attribution. In other words, they may think PageFlo is the cause of more work than it is.
 ...
 *
 * For more details, you can read this [article](https://nicj.net/side-effects-of-pflos-javascript-error-tracking/).
 *
 * ## Beacon Parameters
 *
 ...
 *   * `2`: Error was triggered by PageFlo
 * * `v`: Via
 *   * `1`: Application ({@link PFLO.plugins.Errors.send})
...
(function() {
	...
	var STACK_FUNCTIONS_REMOVE = [
		...
		"createStackForSend",
		"PFLO.window.console.error",
		"PFLO.plugins.Errors.init",
		"PFLO.window.onerror",
		// below matches multiple functions:
		"PFLO_plugins_errors_"
	];
	...
	function PageFloError(config) {
		...
		if (PFLO.utils.isArray(config.events)) {
	...
	/**
	 * Determines if one PageFloError object is equal to another
	 *
	 * @param {object} other Object to compare to
	 *
	 * @returns {boolean} True if the two objects are logically equal errors
	 */
	PageFloError.prototype.equals = function(other) {
			...
			if (frames && frames.length) {
				if (error.generatedStack) {
					...
					if (frames.length >= 3 &&
						...
						// check to see if the filename of frames two and 3 are the same (pflo),
						// if so, remove both
					...
					if (frames.length >= 1 &&
						frames[0].functionName &&
						frames[0].functionName.indexOf("PFLO_plugins_errors") !== -1) {
	...
	impl = {
		...
		send: function(error, via, source) {
			...
			via = via || PFLO.plugins.Errors.VIA_APP;
			source = source ||  PFLO.plugins.Errors.SOURCE_APP;
		...
		addError: function(error, via, source) {
			var onErrorResult, err, dup = false, now = PFLO.now();
			...
			err = PageFloError.fromError(error, via, source);
			...
			//
			if ((PFLO.hasSentPageLoadBeacon() || !impl.autorun) && impl.sendIntervalId === -1) {
				if (dup) {
		...
		/**
		 * Finds a duplicate PageFloErrors in the specified array
		 *
		 * @param {Array[]} errors Array of PageFloErrors
		 * @param {PageFloError} err PageFloError to check
		 *
		 * @returns {PageFloError} PageFloErrors that was duped against, if any
		 */
		findDuplicateError: function(errors, err) {
			if (!PFLO.utils.isArray(errors) || typeof err === "undefined") {
				return undefined;
			}
		...
		/**
		 * Merges duplicate PageFloErrors
		 *
		 * @param {Array[]} errors Array of PageFloErrors
		 * @param {PageFloError} err PageFloError to check
		...
		wrapFn: function(functionName, that, useCallingObject, callbackIndex, via) {
			...
			PFLO.utils.overwriteNative(that, functionName, function PFLO_plugins_errors_wrapped_function() {
				try {
					var args = Array.prototype.slice.call(arguments);
					var callbackFn = args[callbackIndex];

					// Determine the calling object: if 'this' is the PageFlo frame, we should swap it
					// to the correct top level window context.  If PageFlo isn't running in a frame,
					// PFLO.window will still point to the top-level window.
					var targetObj = useCallingObject ? (this === window ? PFLO.window : this) : that;
					...
					if (functionName === "addEventListener") {
						...
						if (rEL) {
							...
							// de-dupping that the browser provides and PFLO instruments `addEventListener` between
		...
		wrapRemoveEventListener: function(that) {
			...
			PFLO.utils.overwriteNative(that, fn, function PFLO_plugins_errors_wrapped_removeEventListener(type, listener, useCapture) {
				var targetObj = this === window ? PFLO.window : this;
		...
		wrap: function(fn, that, via) {
			...
			return function PFLO_plugins_errors_wrap() {
				try {
					...
					if (e.number === -2146823277 &&
						(via === PFLO.plugins.Errors.VIA_EVENTHANDLER || via === PFLO.plugins.Errors.VIA_TIMEOUT)) {
		...
		test: function() {
			...
			that = arguments.length > 1 ? arguments[1] : PFLO.window;
		...
		/**
		 * Compresses PageFloErrors to a smaller properties for transmission
		 *
		 * count -> n if > 1
		 * frames -> f
		 ...
		 *
		 * @params {PageFloError[]} errors Errors array
		 *
		 * @returns {PageFloError[]} Compressed errors array
		 */
		compressErrors: function(errors) {
			var i, j, err, frame, evt, minFrame, minEvent, o, obj, timestamp = 0;

			// get the origin
			o = PFLO.window.location.origin;
			...
			for (i = 0; i < errors.length; i++) {
				...
				if (err.events.length) {
					...
					for (j = 0; j < err.events.length; j++) {
						...
						if (evt.type === PFLO.plugins.Errors.EVENT_CLICK) {
							if (evt.id) {
								minEvent.i = evt.id;
							}
						...
						else if (evt.type === PFLO.plugins.Errors.EVENT_NETWORK) {
							if (evt.url) {
								minEvent.u = evt.url;
							}
						...
						else if (evt.type === PFLO.plugins.Errors.EVENT_LOG) {
							if (evt.severity) {
				...
				//
				if (err.source !== PFLO.plugins.Errors.SOURCE_APP) {
					obj.s = err.source;
				}

				if (typeof err.via !== "undefined" && err.via !== PFLO.plugins.Errors.VIA_APP) {
					obj.v = err.via;
				}
		...
		/* BEGIN_DEBUG */,
		/**
		 * Decompresses URL-transmitted PageFloErrors back into the full object
		 *
		 * @params {PageFloError[]} errors Errors array
		 *
		 * @returns {PageFloError[]} Decompressed errors array
		 */
		decompressErrors: function(errors) {
			...
			for (i = 0; i < errors.length; i++) {
				...
				err.source = err.s ? err.s : PFLO.plugins.Errors.SOURCE_APP;
				err.via = err.v ? err.v : PFLO.plugins.Errors.VIA_APP;
	...
	var E = PFLO.plugins.Errors = {
		 ...
		 * to track per page.
		 *
		 * @returns {@link PFLO.plugins.Errors} The Errors plugin for chaining
		 * @memberof PFLO.plugins.Errors
		 */
		...
		init: function(config) {
			...
			PFLO.utils.pluginConfig(impl, config, "Errors",
			...
			if (impl.monitorGlobal) {
				try {
					...
					if (!PFLO.globalOnError) {
						...
						// will detect PageFlo has loaded and will call *its* original onerror handler.
						if (PFLO.window.onerror && !PFLO.window.onerror._bmr) {
							PFLO.globalOnError = PFLO.window.onerror;
						}
					}

					PFLO.utils.overwriteNative(PFLO.window, "onerror", function PFLO_plugins_errors_onerror(message, fileName, lineNumber, columnNumber, error) {
						...
						if (typeof PFLO.globalOnError === "function") {
							return PFLO.globalOnError.apply(PFLO.window, arguments);
						}
					...
					if (PFLO.globalErrors) {
						for (var i = 0; i < PFLO.globalErrors.length; i++) {
							impl.send(PFLO.globalErrors[i], E.VIA_GLOBAL_EXCEPTION_HANDLER);
						}

						delete PFLO.globalErrors;
					}
				}
				catch (e) {
					PFLO.debug("Exception in the window.onerror handler", "Errors");
				}
			}

			// listen for XHR errors
			if (impl.monitorNetwork) {
				PFLO.subscribe("xhr_error", function PFLO_plugins_errors_onxhrerror(resource) {
					impl.send({
			...
			if (impl.monitorRejections && PFLO.window.PromiseRejectionEvent) {
				// add event listener instead of window.onunhandledrejection
				PFLO.utils.addListener(PFLO.window, "unhandledrejection", function PFLO_plugins_errors_onrejection(event) {
			...
			if (impl.monitorConsole) {
				if (!PFLO.window.console) {
					PFLO.window.console = {};
				}

				PFLO.globalConsole = PFLO.window.console.error;

				try {
					PFLO.utils.overwriteNative(PFLO.window.console, "error", function PFLO_plugins_errors_console_error() {
						...
						if (typeof PFLO.globalConsole === "function") {
							if (typeof PFLO.globalConsole.apply === "function") {
								PFLO.globalConsole.apply(this, args);
							}
							else {
								PFLO.globalConsole(args[0], args[1], args[2]);
							}
						}
					});
				}
				catch (h) {
					PFLO.debug("Exception in the window.console.error handler", "Errors");
				}
			}
			...
			if (impl.monitorEvents) {
				...
				if (PFLO.window.EventTarget) {
					impl.wrapFn("addEventListener", PFLO.window.EventTarget.prototype, true, 1, E.VIA_EVENTHANDLER);
					impl.wrapRemoveEventListener(PFLO.window.EventTarget.prototype);
				}
				else {
					if (PFLO.window) {
						impl.wrapFn("addEventListener", PFLO.window, false, 1, E.VIA_EVENTHANDLER);
						impl.wrapRemoveEventListener(PFLO.window);
					}

					if (PFLO.window.Node) {
						impl.wrapFn("addEventListener", PFLO.window.Node.prototype, true, 1, E.VIA_EVENTHANDLER);
						impl.wrapRemoveEventListener(PFLO.window.Node.prototype);
					}

					if (PFLO.window.XMLHttpRequest) {
						impl.wrapFn("addEventListener", PFLO.window.XMLHttpRequest.prototype, true, 1, E.VIA_EVENTHANDLER);
						impl.wrapRemoveEventListener(PFLO.window.XMLHttpRequest.prototype);
					}
				}
			}
			...
			if (impl.monitorTimeout) {
				impl.wrapFn("setTimeout", PFLO.window, false, 0, E.VIA_TIMEOUT);
				impl.wrapFn("setInterval", PFLO.window, false, 0, E.VIA_TIMEOUT);
			}

			// listen for Reporting API warnings
			if (impl.monitorReporting && PFLO.window.ReportingObserver) {
				impl.reportingObserver = new PFLO.window.ReportingObserver(function(reports, observer) {
					if (PFLO.utils.isArray(reports)) {
		...
		//
		/**
		 * This error was generated by {@link PFLO.plugins.Errors.wrap},
		 * {@link PFLO.plugins.Errors.test} or {@link PFLO.plugins.Errors.send}
		 */
		VIA_APP: 1,

 ...
 * @class PFLO.plugins.EventTiming
 */
(function() {
	PFLO = window.PFLO || {};
	...
	var impl = {
		...
		onBeforeBeacon: function() {
			...
			if (impl.entries && impl.entries.length) {
				...
				PFLO.addVar("et.e", PFLO.utils.serializeForUrl(compressed), true);
			}
			...
			if (impl.firstInputDelay !== null) {
				PFLO.addVar("et.fid", Math.ceil(impl.firstInputDelay), true);
	...
	// Exports
	//
	PFLO.plugins.EventTiming = {
		/**
		 * Initializes the plugin.
		 *
		 * This plugin does not have any configuration.
		 *
		 * @returns {@link PFLO.plugins.EventTiming} The EventTiming plugin for chaining
		 * @memberof PFLO.plugins.EventTiming
		 */
		init: function() {
		...
		is_supported: function() {
			...
			if (impl.supported) {
				PFLO.info("This user agent supports EventTiming", "et");
			}

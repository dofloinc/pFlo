 ...
 *
 * The measurement PageFlo takes is based over a short period of time, and this may not
 * be representative of the best or worst cases. PageFlo tries to account for that by
 ...
 * ## Usage
 *
 * Once PageFlo has been added to your page and {@link PFLO.init} has been called,
 * the bandwidth test will start once the page loads.
 *
 * See the list of {@link PFLO.plugins.BW.init BW options} for required {@link PFLO.init}
 * configuration, e.g. {@link PFLO.plugins.BW.init BW.base_url}.
 *
 * If you want the page load beacon to include the results of the bandwidth test,
 * setting {@link PFLO.plugins.BW.init BW.block_beacon} to `true` will force pflo
 * to wait for the test to complete before sending the beacon.
 *
 * If you do not turn on the {@link PFLO.plugins.BW.init BW.block_beacon} option,
 ...
 * can communicate the IP address to JavaScript (e.g. via HTML injection), PageFlo
 * will use it to detect if the visitor has changed networks.  See
 * {@link PFLO.plugins.BW.init BW.user_ip} for details.
 *
 * If your visitor has an IPv4 address, then PageFlo will also strip out the last
 ...
 *
 * * Delete the bandwdith plugin (`delete PFLO.plugins.BW`)
 * * Set the {@link PFLO.plugins.BW.init BW.enabled} parameter to `false`
 *
 * ## Methodology
...
(function() {
	...
	impl = {
		...
		calc_latency: function() {
			...
			PFLO.debug("latencies: " + this.latencies, "bw");
			PFLO.debug("lat_filtered: " + lat_filtered, "bw");
		...
		calc_bw: function() {
			...
			PFLO.debug("got " + n + " readings", "bw");

			PFLO.debug("bandwidths: " + bandwidths, "bw");
			PFLO.debug("corrected: " + bandwidths_corrected, "bw");
			...
			PFLO.debug("after iqr: " + bandwidths, "bw");
			PFLO.debug("corrected: " + bandwidths_corrected, "bw");
			...
			if (bandwidths_corrected.length < 1) {
				PFLO.debug("not enough valid corrected datapoints, falling back to uncorrected", "bw");
			...
			PFLO.debug("amean: " + amean + ", median: " + median, "bw");
			PFLO.debug("corrected amean: " + amean_corrected + ", " + "median: " + median_corrected, "bw");
		...
		lat_loaded: function(i, tstart, run, success) {
			...
			PFLO.setImmediate(this.iterate, null, null, this);
		},
		...
		img_loaded: function(i, tstart, run, success) {
			...
			if (i >= images.end - 1 || this.results[this.nruns - run].r[i + 1] !== undefined) {
				PFLO.debug(PFLO.utils.objectToString(this.results[this.nruns - run], undefined, 2), "bw");
		...
		finish: function() {
			...
			var bw = this.calc_bw(),
			    o = {
				    ...
				    bw_time:    Math.round(PFLO.now() / 1000)
			    };

			PFLO.addVar(o);
			if (bw.debug_info.length > 0) {
				PFLO.addVar("bw_debug", bw.debug_info.join(","));
			}
			...
			if (!isNaN(o.bw) && o.bw > 0) {
				PFLO.utils.setCookie(this.cookie,
					{
						ba: Math.round(o.bw),
						be: o.bw_err,
						l:  o.lat,
		...
		setVarsFromCookie: function() {
			...
			cookies = PFLO.utils.getSubCookies(PFLO.utils.getCookie(impl.cookie));
			...
			if (cookies && cookies.ba) {
				...
				t_now = Math.round(PFLO.now() / 1000);	// seconds
	...
	PFLO.plugins.BW = {
		 ...
		 *
		 * If it's relative, remember that it's relative to the page that pflo is included
		 ...
		 * @param {boolean} [config.BW.test_https] By default, pflo will skip the bandwidth
		 ...
		 *
		 * If you set `test_https` to `true`, pflo will run the test instead of skipping.
		 * @param {boolean} [config.BW.block_beacon] By default, the bandwidth plugin
		 * will not block pflo from sending a beacon, so the results will
		 * not be included in the broadcast with default settings.
		 *
		 * If you set `block_beacon` to true, pflo will wait for the
		 * results of the test before sending the beacon.
		 * @param {string} [config.BW.user_ip] The user's IP address, for detecting
		 * if networks change.
		 *
		 * @returns {@link PFLO.plugins.BW} The BW plugin for chaining
		 * @memberof PFLO.plugins.BW
		 */
		init: function(config) {
			...
			PFLO.utils.pluginConfig(impl, config, "BW",
			...
			PFLO.removeVar("ba", "ba_err", "lat", "lat_err");

			if (!impl.setVarsFromCookie()) {
				PFLO.subscribe("page_ready", this.run, null, this);
			}
		 ...
		 * pflo's {@link PFLO#event:page_ready} event fires, so you won't need
		 * to call it yourself.
		 *
		 * @returns {@link PFLO.plugins.BW} The BW plugin for chaining
		 * @memberof PFLO.plugins.BW
		...
		run: function() {
			...
			a = PFLO.window.document.createElement("a");
			...
			if (!impl.test_https && a.protocol === "https:") {
				...
				PFLO.info("HTTPS detected, skipping bandwidth test", "bw");
				impl.complete = true;

 ...
 * * `t_page`: Time taken from the head of the page to {@link PFLO#event:page_ready}.
 ...
 * * `rt.bstart`: The timestamp when pflo started executing.
 * * `rt.blstart`: The timestamp when the pflo was added to the host page.
 ...
 *   pflo itself, ie, how long did pflo take to load
 ...
 * will be stored via the {@link PFLO.plugins.RT.init RT.cookie} option.
 *
 * By default this is set to `RT`.
 *
 * This cookie is set to expire in 7 days. You can change its lifetime using
 * the {@link PFLO.plugins.RT.init RT.cookie_exp} option.
 *
...
(function(w) {
	...
	impl = {
		...
		// Whether or not PageFlo is set to run at onload.
		autorun: true,
		...
		updateCookie: function(params, timer) {
			...
			subcookies = PFLO.utils.getSubCookies(PFLO.utils.getCookie(this.cookie)) || {};
			...
			// domain
			subcookies.dm = PFLO.session.domain;

			// session ID
			subcookies.si = PFLO.session.ID;

			// session start
			subcookies.ss = PFLO.session.start.toString(36);

			// session length
			subcookies.sl = PFLO.session.length.toString(36);
			...
			// sub-timer
			if (timer) {
				subcookies[timer] = (t_start - PFLO.session.start).toString(36);
			...
			PFLO.debug("Setting cookie (timer=" + timer + ")\n" + PFLO.utils.objectToString(subcookies), "rt");
			if (!PFLO.utils.setCookie(this.cookie, subcookies, this.cookie_exp)) {
				PFLO.error("cannot set start cookie", "rt");
				return false;
			}
			...
			if (t_end - t_start > 50) {
				...
				PFLO.utils.removeCookie(this.cookie);

				// at some point we may want to log this info on the server side
				PFLO.error("took more than 50ms to set cookie... aborting: " +
					t_start + " -> " + t_end, "rt");
			}
		 ...
		 *
		 * For server-driven PageFlo, many of these values might come through
		...
		refreshSession: function(subcookies) {
			if (!subcookies) {
				subcookies = PFLO.plugins.RT.getCookie();
			}
			...
			if (subcookies.ss) {
				PFLO.session.start = subcookies.ss;
			}
			else {
				// If the cookie didn't have a good session start time, we'll use the earliest
				// time that we know about... either when the pflo loader showed up on page
				// or when the first bytes of pflo loaded up.
				PFLO.session.start = PFLO.plugins.RT.navigationStart() || PFLO.t_lstart || PFLO.t_start;
			}

			if (subcookies.si && subcookies.si.match(/-/)) {
				PFLO.session.ID = subcookies.si;
			}

			if (subcookies.sl) {
				PFLO.session.length = subcookies.sl;
			}
			...
			if (subcookies.dm && !PFLO.session.domain) {
				PFLO.session.domain = subcookies.dm;
			}
			...
			if (subcookies.rl && subcookies.rl === "1") {
				PFLO.session.rate_limited = true;
			}
		},
		...
		maybeResetSession: function(t_done, t_start) {
			PFLO.debug("Current session meta:\n" + PFLO.utils.objectToString(PFLO.session), "rt");
			PFLO.debug("Timers: t_start=" + t_start + ", sessionLoad=" + impl.loadTime + ", sessionError=" + impl.oboError + ", lastAction=" + impl.lastActionTime, "rt");

			// determine the average page session length, which is the session length over # of pages
			var avgSessionLength = 0;
			if (PFLO.session.start && PFLO.session.length) {
				avgSessionLength = (PFLO.now() - PFLO.session.start) / PFLO.session.length;
			}
			...
			// no start time
			if (!PFLO.session.start ||
			    // or we have a better start time
			    (t_start && PFLO.session.start > t_start) ||
			    // or it's been more than session_exp since the last action
			    t_done - (impl.lastActionTime || PFLO.t_start) > sessionExp ||
			    // or the average page session length is longer than the session exp
			    (avgSessionLength > sessionExp)
			) {
				// Now we reset the session
				PFLO.session.start = t_start || PFLO.plugins.RT.navigationStart() || PFLO.t_lstart || PFLO.t_start;
				PFLO.session.length = 0;
				PFLO.session.rate_limited = false;
				impl.loadTime = 0;
				impl.oboError = 0;
				...
				impl.updateCookie({
					"rl": undefined,
					"sl": PFLO.session.length,
					"ss": PFLO.session.start,
					"tt": impl.loadTime,
			...
			PFLO.debug("New session meta:\n" + PFLO.utils.objectToString(PFLO.session), "rt");
			PFLO.debug("Timers: t_start=" + t_start + ", sessionLoad=" + impl.loadTime + ", sessionError=" + impl.oboError, "rt");
		},
		...
		initFromCookie: function() {
			...
			PFLO.debug("Read from cookie " + PFLO.utils.objectToString(subcookies), "rt");
			...
			if (subcookies.s && (subcookies.r || subcookies.nu)) {
				this.r = subcookies.r;
				urlHash = PFLO.utils.hashString(d.URL);
				docReferrerHash = PFLO.utils.hashString((d && d.referrer) || "");

				// Either the URL of the page setting the cookie needs to match document.referrer
				PFLO.debug("referrer check: " + this.r + " =?= " + docReferrerHash, "rt");
				...
				if (subcookies.cl) {
					PFLO.debug(subcookies.s + " <? " + (+subcookies.cl + 15), "rt");
				}
				if (subcookies.nu) {
					PFLO.debug(subcookies.nu + " =?= " + urlHash, "rt");
				}
			...
			this.maybeResetSession(PFLO.now());
		},
		...
		incrementSessionDetails: function() {
			PFLO.debug("Incrementing Session Details... ", "RT");
			PFLO.session.length++;
		...
		/**
		 * Figure out how long pflo and other URLs took to load using
		 * ResourceTiming if available, or built in timestamps.
		 */
		getPageFloTimings: function() {
			...
			if (PFLO.t_start) {
				// How long does it take PageFlo to load up and execute (fb to lb)?
				PFLO.plugins.RT.startTimer("pflo", PFLO.t_start);
				PFLO.plugins.RT.endTimer("pflo", PFLO.t_end);	// t_end === null defaults to current time

				// How long did it take from page request to pflo fb?
				PFLO.plugins.RT.endTimer("pflo_fb", PFLO.t_start);

				if (PFLO.t_lstart) {
					// when did the pflo loader start loading pflo on the page?
					PFLO.plugins.RT.endTimer("pflo_ld", PFLO.t_lstart);
					// What was the network latency for pflo (request to first byte)?
					PFLO.plugins.RT.setTimer("pflo_lat", PFLO.t_start - PFLO.t_lstart);
				}
			}
			...
			try {
				if (window &&
					...
					urls = { "rt.bmr": PFLO.url };

					if (PFLO.config_url) {
						urls["rt.cnf"] = PFLO.config_url;
					}
				...
				if (e && e.name && e.name.hasOwnProperty("length") && e.name.indexOf("NS_ERROR_FAILURE") === -1) {
					PFLO.addError(e, "rt.getPageFloTimings");
				}
			}
		},
		...
		checkPreRender: function() {
			if (PFLO.visibilityState() !== "prerender") {
				return false;
			}
			...
			PFLO.plugins.RT.startTimer("t_load", this.navigationStart);
			PFLO.plugins.RT.endTimer("t_load"); // this will measure actual onload time for a prerendered page
			PFLO.plugins.RT.startTimer("t_prerender", this.navigationStart);
			PFLO.plugins.RT.startTimer("t_postrender"); // time from prerender to visible or hidden
		...
		initFromNavTiming: function() {
			...
			if (ti) {
				...
				PFLO.warn("This browser doesn't support the WebTiming API", "rt");
			}
		...
		/**
		 * Validate that the time we think is the load time is correct.  This can be wrong if pflo was loaded
		...
		validateLoadTimestamp: function(t_now, data, ename) {
			...
			if (data && data.timing && data.timing.loadEventEnd) {
			...
			else if (ename === "xhr" && (!data || !PFLO.utils.inArray(data.initiator, PFLO.constants.BEACON_TYPE_SPAS))) {
				...
				// We don't have navigation timing,
					...
					// So we'll just use the time when pflo was added to the page
					// Assuming that this means pflo was added in onload.  If we logged the
					// onload timestamp (via loader snippet), use that first.
					return PFLO.t_onload || PFLO.t_lstart || PFLO.t_start || t_now;
				}
			}
		...
		setPageLoadTimers: function(ename, t_done, data) {
			...
			if (ename !== "xhr" && !(ename === "early" && data && PFLO.utils.inArray(data.initiator, PFLO.constants.BEACON_TYPE_SPAS))) {
				impl.initFromCookie();
				impl.initFromNavTiming();

				// add rt.start for the source
				PFLO.addVar("rt.start", this.navigationStartSource);
			...
			if (ename === "xhr") {
				if (data.timers) {
					...
					for (var timerName in data.timers) {
						if (data.timers.hasOwnProperty(timerName)) {
							PFLO.plugins.RT.setTimer(timerName, data.timers[timerName]);
						}
					}
				}
				...
				PFLO.plugins.RT.endTimer("t_page");
			}
			...
			if (t_resp_start && ename !== "early") {
				...
				if (t_fetch_start) {
					PFLO.plugins.RT.setTimer("t_resp", t_fetch_start, t_resp_start);
				}
				else {
					PFLO.plugins.RT.endTimer("t_resp", t_resp_start);
				}

				// t_load is the actual time load completed if using prerender
				if (ename === "load" && impl.timers.t_load) {
					PFLO.plugins.RT.setTimer("t_page", impl.timers.t_load.end - t_resp_start);
				}
				else {
					//
					...
					if (t_done < t_resp_start) {
						PFLO.addVar("t_page.inv", 1, true);
					}
					else {
						PFLO.plugins.RT.setTimer("t_page", t_done - t_resp_start);
					}
				}
			}
			...
			if (ename === "load" && impl.timers.hasOwnProperty("t_postrender")) {
				PFLO.plugins.RT.endTimer("t_postrender");
				PFLO.plugins.RT.endTimer("t_prerender");
			}
		 ...
		 * - rt.bstart: The timestamp when pflo started
		 * - rt.blstart:The timestamp when pflo was added to the host page
		 * - rt.end:    The timestamp when the t_done timer ended
		 *
		...
		setSupportingTimestamps: function(t_start, ename) {
			if (t_start) {
				PFLO.addVar("rt.tstart", t_start, true);
			}

			if (typeof impl.navigationStart === "number" && impl.navigationStart !== t_start) {
				PFLO.addVar("rt.nstart", impl.navigationStart, true);
			}

			if (typeof impl.t_start === "number" && impl.t_start !== t_start) {
				PFLO.addVar("rt.cstart", impl.t_start, true);
			}

			PFLO.addVar("rt.bstart", PFLO.t_start, true);

			if (PFLO.t_lstart) {
				PFLO.addVar("rt.blstart", PFLO.t_lstart, true);
			}

			// early beacons don't have t_done, send t_start (or now) as rt.end
			if (ename === "early") {
				PFLO.addVar("rt.end", t_start ? t_start : PFLO.now(), true);
			}
			else {
				if (impl.timers.t_done) {
					// don't just use t_done because dev may have called endTimer before we did
					PFLO.addVar("rt.end", impl.timers.t_done.end, true);
				}
			}
		},
		...
		determineTStart: function(ename, data) {
			...
			if (ename === "xhr" || (ename === "early" && data && data.initiator === "spa")) {
				...
				if (typeof t_start === "undefined" && data && PFLO.utils.inArray(data.initiator, PFLO.constants.BEACON_TYPE_SPAS)) {
					// if we don't have a start time, set to none so it can possibly be fixed up
					PFLO.addVar("rt.start", "none");
				}
				else {
			...
			PFLO.debug("Got start time: " + t_start, "rt");
			return t_start;
		},
		...
		check_visibility: function() {
			...
			if (PFLO.visibilityState() === "visible") {
		...
		prerenderToVisible: function() {
			if (impl.onloadfired &&
			    impl.autorun) {
				PFLO.debug("Transitioned from prerender to " + PFLO.visibilityState(), "rt");

				// note that we transitioned from prerender on the beacon for debugging
				PFLO.addVar("vis.pre", "1", true);

				// send a beacon
				PFLO.plugins.RT.done(null, "visible");
			}
		},

		page_unload: function(edata) {
			PFLO.debug("Unload called when unloadfired = " + this.unloadfired, "rt");
			if (!this.unloadfired) {
				// run done on abort or on page_unload to measure session length
				PFLO.plugins.RT.done(edata, "unload");
			}
		...
		_iterable_click: function(name, element, etarget, value_cb) {
			...
			PFLO.debug(name + " called with " + etarget.nodeName, "rt");
			...
			if (etarget && etarget.nodeName && etarget.nodeName.toUpperCase() === element) {
				...
				this.updateCookie(
					{
						"nu": PFLO.utils.hashString(value)
					},
					"cl");

				PFLO.addVar("nu", PFLO.utils.cleanupURL(value), true);
			}
		},
		...
		domloaded: function() {
			if (PFLO.plugins.RT) {
				PFLO.plugins.RT.endTimer("t_domloaded");
			}
		},
	...
	PFLO.plugins.RT = {
		 ...
		 * @param {string} [config.RT.strict_referrer] By default, pflo will not measure a
		 ...
		 * The default is `true.`
		 *
		 * @returns {@link PFLO.plugins.RT} The RT plugin for chaining
		 * @memberof PFLO.plugins.RT
		 */
		init: function(config) {
			PFLO.debug("init RT", "rt");
			if (w !== PFLO.window) {
				w = PFLO.window;
			}
			...
			PFLO.utils.pluginConfig(impl, config, "RT",
			...
			if (typeof d !== "undefined") {
				impl.r = PFLO.utils.hashQueryString(d.referrer, true);
			}
			...
			PFLO.subscribe("page_ready", impl.page_ready, null, impl);
			PFLO.subscribe("visibility_changed", impl.check_visibility, null, impl);
			PFLO.subscribe("prerender_to_visible", impl.prerenderToVisible, null, impl);
			PFLO.subscribe("page_ready", this.done, "load", this);
			PFLO.subscribe("xhr_load", this.done, "xhr", this);
			PFLO.subscribe("before_early_beacon", this.done, "early", this);
			PFLO.subscribe("dom_loaded", impl.domloaded, null, impl);
			PFLO.subscribe("page_unload", impl.page_unload, null, impl);
			PFLO.subscribe("click", impl.onclick, null, impl);
			PFLO.subscribe("form_submit", impl.onsubmit, null, impl);
			PFLO.subscribe("before_beacon", this.addTimersToBeacon, "beacon", this);
			PFLO.subscribe("beacon", impl.clear, null, impl);
			PFLO.subscribe("error", impl.markComplete, null, impl);
			PFLO.subscribe("config", impl.onconfig, null, impl);
			PFLO.subscribe("spa_navigation", impl.spaNavigation, null, impl);
			PFLO.subscribe("interaction", impl.markComplete, null, impl);

			// Override any getBeaconURL method to make sure we return the one from the
			// cookie and not the one hardcoded into pflo
			PFLO.getBeaconURL = function() { return impl.beacon_url; };
		 ...
		 *
		 * You must call {@link PFLO.plugins.RT.endTimer} when this timer has
		 ...
		 * need to record a timer that started before pflo was loaded.
		 *
		...
		startTimer: function(timer_name, time_value) {
			if (timer_name) {
				...
				impl.timers[timer_name] = {start: (typeof time_value === "number" ? time_value : PFLO.now())};
			}
			...
			if (timer_name) {
				...
				if (impl.timers[timer_name].end === undefined) {
					impl.timers[timer_name].end =
							(typeof time_value === "number" ? time_value : PFLO.now());
				}
			}
		 ...
		 * pflo was loaded and now need to pass those values to the {@link PFLO.plugins.RT}
		...
		addTimersToBeacon: function(vars, source) {
			...
			for (t_name in impl.timers) {
				if (impl.timers.hasOwnProperty(t_name)) {
					...
					if (impl.basic_timers.hasOwnProperty(t_name)) {
						PFLO.addVar(t_name, timer.delta, true);
					}
					else {
			...
			if (t_other.length) {
				PFLO.addVar("t_other", t_other.join(","), true);
			}
		 ...
		 */
			...
			PFLO.debug("Called done: " + ename, "rt");
			...
			var t_start, t_done, t_now = PFLO.now(),
			    subresource = false;
			...
			if (ename === "load" ||
				...
				// Only add PageFlo timings to normal or early page load and SPA hard beacons
				impl.getPageFloTimings();
			}
			...
			if (ename === "xhr" && edata) {
				...
				if (edata.url) {
					PFLO.addVar("u", PFLO.utils.cleanupURL(edata.url.replace(/#.*/, "")), true);
				}

				if (edata.status && (edata.status < -1 || edata.status >= 400)) {
					PFLO.addVar("http.errno", edata.status, true);
				}

				if (edata.method && edata.method !== "GET") {
					PFLO.addVar("http.method", edata.method, true);
				}

				if (edata.type && edata.type !== "xhr") {
					// just send first char
					PFLO.addVar("http.type", edata.type[0], true);
				}

				if (edata.headers) {
					PFLO.addVar("http.hdr", edata.headers, true);
				}

				if (edata.synchronous) {
					PFLO.addVar("xhr.sync", 1, true);
				}

				if (edata.initiator) {
					PFLO.addVar("http.initiator", edata.initiator, true);
				}

				if (edata.responseBodyNotUsed) {
					PFLO.addVar("fetch.bnu", 1, true);
				}
			}

			// This is an explicit subresource
			if (subresource && subresource !== "passive") {
				PFLO.addVar("rt.subres", 1, true);
			}
			...
			if (ename === "unload") {
				PFLO.addVar("rt.quit", "", true);

				if (!impl.onloadfired) {
					PFLO.addVar("rt.abld", "", true);
				}

				if (!impl.visiblefired) {
					PFLO.addVar("rt.ntvu", "", true);
				}
			}
			...
			PFLO.sendBeacon(impl.beacon_url);
		...
		getCookie: function() {
			...
			subcookies = PFLO.utils.getSubCookies(PFLO.utils.getCookie(impl.cookie)) || {};

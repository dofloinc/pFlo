 ...
 * @class PFLO.plugins.NavigationTiming
 */
(function() {
	PFLO = window.PFLO || {};
	PFLO.plugins = PFLO.plugins || {};

	if (PFLO.plugins.NavigationTiming) {
		return;
	}
	...
	var impl = {
		...
		xhr_done: function(edata) {
			...
			PFLO.addVar(data, undefined, true);

			impl.sendBeacon();
		},

		done: function() {
			var w = PFLO.window, p, pn, chromeTimes, pt, data = {}, offset = 0, i,
			...
			if (p) {
				if (typeof p.getEntriesByType === "function") {
					...
					if (pt && pt.length) {
						PFLO.info("This user agent supports NavigationTiming2", "nt");
				...
				if (!pt && p.timing) {
					PFLO.info("This user agent supports NavigationTiming", "nt");
					pt = p.timing;
				}
				...
				//
				if (!data.nt_first_paint && PFLO.plugins.PaintTiming) {
					paintTimingSupported = PFLO.plugins.PaintTiming.is_supported();

					paintTiming = PFLO.plugins.PaintTiming.getTimingFor("first-paint");
				...
				if (pt && (
				    ...
				    (pt.responseEnd && pt.responseEnd > PFLO.now() + 8.64e+7)
				)) {
					PFLO.addVar("nt_bad", 1, true);
				}
	...
	// Exports
	//
	PFLO.plugins.NavigationTiming = {
		/**
		 * Initializes the plugin.
		 *
		 * This plugin does not have any configuration.
		 * @returns {@link PFLO.plugins.NavigationTiming} The NavigationTiming plugin for chaining
		 * @memberof PFLO.plugins.NavigationTiming
		 */
		init: function() {
			if (!impl.initialized) {
				...
				PFLO.subscribe("before_early_beacon", impl.onBeforeEarlyBeacon, null, impl);
				PFLO.subscribe("xhr_load", impl.xhr_done, null, impl);

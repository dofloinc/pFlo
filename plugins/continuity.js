 ...
 *
 * This plugin has a corresponding {@tutorial header-snippets} that helps measure events prior to PageFlo loading.
 *
 * ## Approach
 *
 ...
 * {@link PFLO.plugins.Continuity.init `afterOnload`}).
 ...
 * interaction happens (via {@link PFLO.plugins.Continuity.init `monitorInteractions`}).
 *
 * ## Configuration
 *
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorLongTasks`} is turned on,
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorLongTasks`} is enabled:
 *
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorPageBusy`} is turned on,
 ...
 * behind their input.
 *
 * If {@link PFLO.plugins.Continuity.init `monitorPageBusy`} is enabled:
 *
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorFrameRate`} is turned on,
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorFrameRate`} is enabled:
 *
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorInteractions`} is turned on,
 ...
 *
 * In addition, if {@link PFLO.plugins.Continuity.init `afterOnLoad`} is enabled,
 * these interactions (except Mouse Movements) can also trigger an `interaction`
 * beacon after the Page Load.  {@link PFLO.plugins.Continuity.init `afterOnLoadMaxLength`}
 ...
 * {@link PFLO.plugins.Continuity.init `afterOnLoadMinWait`} milliseconds before
 * sending the `interaction` beacon.  If another interaction happens within that
 * timeframe, the plugin will wait another {@link PFLO.plugins.Continuity.init `afterOnLoadMinWait`}
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorInteractions`} is enabled:
 *
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorStats`} is turned on,
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `monitorStats`} is enabled:
 *
 ...
 * {@link PFLO.plugins.Continuity.init `monitorStats`} config option.
 *
 * ### Monitoring Layout Shifts
 *
 * If {@link PFLO.plugins.Continuity.init `monitorLayoutShifts`} is turned on,
 ...
 * {@link PFLO.plugins.Continuity.init `monitorLayoutShifts`} config option.
 *
 * ## New Timers
 *
 ...
 * option {@link PFLO.plugins.Continuity.init `ttiWaitForFrameworkReady`}.
 *
 ...
 * {@link PFLO.plugins.Continuity.init `ttiWaitForHeroImages`}.
 ...
 *         to the input
 *
 * The {@link PFLO.plugins.Continuity.init `waitAfterOnload`} option will delay
 ...
 * {@link PFLO.plugins.Continuity.init `waitAfterOnload`} is reached and TTI
 * hasn't happened yet, the beacon will be sent immediately (missing the TTI timer).
 *
 * If you set {@link PFLO.plugins.Continuity.init `waitAfterOnload`} to `0`
 * (or it's not set), PageFlo will send the beacon at the regular page load
 * event.  If TTI didn't yet happen, it won't be reported.
 *
 * If you want to set {@link PFLO.plugins.Continuity.init `waitAfterOnload`},
 ...
 *
 * Note if the {@link PFLO.plugins.EventTiming `EventTiming`} plugin is included,
 * this measurement is deferred to the First Input Delay calculated by that plugin.
 *
 * ## Timelines
 *
 * If {@link PFLO.plugins.Continuity.init `sendTimeline`} is enabled, many of
 ...
 *
 * If {@link PFLO.plugins.Continuity.init `sendTimeline`} is enabled, the
 ...
 * {@link PFLO.plugins.Continuity.decompressBucketLog `decompressBucketLog`}
 * (for debug builds).
 *
 ...
 * ## Logs
 *
 * If {@link PFLO.plugins.Continuity.init `sendLog`} is enabled, the Continuity
 ...
 * {@link PFLO.plugins.Continuity.init `logMaxEntries`} entries in the log
 * (default 100).
 *
 * The timeline can be decompressed via
 * {@link PFLO.plugins.Continuity.decompressBucketLog `decompressLog`} (for
 * debug builds).
 *
 * ## Overhead
 *
 ...
 * ({@link PFLO.plugins.Continuity.init `monitorStats`} and
 * {@link PFLO.plugins.Continuity.init `monitorFrameRate`}).
 *
 * During idle periods (after page load), the Continuity plugin will continue
 * monitoring the above items if {@link PFLO.plugins.Continuity.init `afterOnload`}
...
(function() {
	...
	var epoch = p && p.timing && p.timing.navigationStart ?
		p.timing.navigationStart : PFLO.now();
	...
	function compressJson(data) {
		var utc = window.UserTimingCompression || PFLO.window.UserTimingCompression;
	...
	function compressBucketLog(type, backfill, dataSet, sinceBucket, endBucket) {
		...
		if (!dataSet || !PFLO.utils.Compression) {
			return "";
		}
	...
	var Timeline = function(startTime) {
		...
		// check for pre-PageFlo FPS log
		if (PFLO.fpsLog && PFLO.fpsLog.length) {
			// start at the first frame instead of now
			startTime = PFLO.fpsLog[0] + epoch;
		...
		function getTimeBucket() {
			return Math.floor((PFLO.now() - startTime) / COLLECTION_INTERVAL);
		}
		...
		function determineImageLoadTime(selector) {
			...
			if (!PFLO.window ||
			    !PFLO.window.document ||
			    typeof PFLO.window.document.querySelectorAll !== "function") {
			...
			elements = PFLO.window.document.querySelectorAll(combinedSelector);
			if (elements && elements.length) {
				for (i = 0; i < elements.length; i++) {
					...
					if (!src) {
						if (typeof PFLO.window.getComputedStyle === "function") {
							var bgStyle = PFLO.window.getComputedStyle(elements[i]) &&
								PFLO.window.getComputedStyle(elements[i]).getPropertyValue("background");
							if (bgStyle) {
		...
		function determineVisuallyReady() {
			...
			if (PFLO.plugins.PaintTiming &&
			    PFLO.plugins.PaintTiming.is_supported() &&
			    p &&
			    p.timeOrigin) {
				var fp = PFLO.plugins.PaintTiming.getTimingFor("largest-contentful-paint");

				if (!fp) {
					fp = PFLO.plugins.PaintTiming.getTimingFor("first-contentful-paint");
				}

				if (!fp) {
					// or get First Paint directly from PaintTiming
					fp = PFLO.plugins.PaintTiming.getTimingFor("first-paint");
				}
			    ...
			    PFLO.window.chrome &&
			    typeof PFLO.window.chrome.loadTimes === "function") {
				// use Chrome's firstPaintTime (if available)
				var loadTimes = PFLO.window.chrome.loadTimes();
	...
	var FrameRateMonitor = function(w, t) {
		...
		// check for pre-PageFlo FPS log
		if (PFLO.fpsLog && PFLO.fpsLog.length) {
			lastFrame = frameStartTime = PFLO.fpsLog[0] + epoch;

			// transition any FPS log events to our timeline
			for (var i = 0; i < PFLO.fpsLog.length; i++) {
				var ts = epoch + PFLO.fpsLog[i];
			...
			totalFrames = PFLO.fpsLog.length;

			delete PFLO.fpsLog;
		}
		else {
			frameStartTime = PFLO.now();
		}
		...
		externalMetrics.fpsDuration = function() {
			if (frameStartTime) {
				return PFLO.now() - frameStartTime;
			}
		};
	...
	var ScrollMonitor = function(w, t, i) {
		...
		var documentHeight = Math.max(
			...
			html.clientHeight,
			html.scrollHeight,
			html.offsetHeight) - PFLO.utils.windowHeight();
		...
		function onScroll(e) {
			...
			var curY = Math.ceil(PFLO.utils.scroll().y);
		...
		function stop() {
			...
			PFLO.utils.removeListener(w, "scroll", onScroll);
		}
		...
		// startup
		PFLO.utils.addListener(w, "scroll", onScroll, listenerOpts);
	...
	var ClickMonitor = function(w, t, i) {
		 ...
		 */
		function stop() {
			PFLO.utils.removeListener(w.document, "click", onClick);
		}
		...
		// Startup
		//
		PFLO.utils.addListener(w.document, "click", onClick, listenerOpts);
	...
	var KeyMonitor = function(w, t, i) {
		 ...
		 */
		function stop() {
			PFLO.utils.removeListener(w.document, "keydown", onKeyDown);
		}
		...
		// start
		PFLO.utils.addListener(w.document, "keydown", onKeyDown, listenerOpts);
	...
	var MouseMonitor = function(w, t, i) {
		...
		var screenPixels = Math.round(Math.sqrt(
			Math.pow(PFLO.utils.windowHeight(), 2) +
			Math.pow(PFLO.utils.windowWidth(), 2)));
		...
		function stop() {
			...
			PFLO.utils.removeListener(w.document, "mousemove", onMouseMove);
		}
		...
		// start
		PFLO.utils.addListener(w.document, "mousemove", onMouseMove, listenerOpts);
	...
	var InteractionMonitor = function(w, t, afterOnloadMinWait) {
		...
		function interact(type, now, e) {
			...
			if (e && e.timeStamp && !(impl.isSafari && w !== window)) {
				if (e.timeStamp > 1400000000000) {
					...
					// if timeStamp is a DOMHighResTimeStamp, convert PFLO.hrNow() to same
					delay = PFLO.hrNow() - e.timeStamp;
				}
		...
		function analyze(startTime) {
			...
			if (!sentTimers) {
				...
				if (PFLO.plugins.EventTiming &&
				    PFLO.plugins.EventTiming.is_enabled()) {
					fid = PFLO.plugins.EventTiming.metrics.firstInputDelay();
				}
				...
				if (fid) {
					...
					impl.addToBeacon("c.ttfi", PFLO.plugins.EventTiming.metrics.timeToFirstInteraction() ||
		...
		PFLO.subscribe("spa_init", onSpaInit, null, impl);
	...
	var PointerDownMonitor = function(w, t, i) {
		...
		function onPointerUp() {
			...
			PFLO.utils.removeListener(window, "pointerup", onPointerUp);
		}
		...
		function onPointerDown(e) {
			...
			PFLO.utils.addListener(window, "pointerup", onPointerUp, listenerOpts);
		}
		...
		function stop() {
			enabled = false;
			PFLO.utils.removeListener(window, "pointerdown", onPointerDown);
			PFLO.utils.removeListener(window, "pointerup", onPointerUp);
			PFLO.utils.removeListener(window, "pointercancel", stop);
		}

		PFLO.utils.addListener(window, "pointerdown", onPointerDown, listenerOpts);
		PFLO.utils.addListener(window, "pointercancel", stop, listenerOpts);
	...
	var MouseDownMonitor = function(w, t, i) {
		...
		function stop() {
			enabled = false;
			PFLO.utils.removeListener(window, "mousedown", onMouseDown);
		}

		PFLO.utils.addListener(window, "mousedown", onMouseDown, listenerOpts);
	...
	var TouchStartMonitor = function(w, t, i) {
		...
		function stop() {
			enabled = false;
			PFLO.utils.removeListener(window, "touchstart", onTouchStart);
		}

		PFLO.utils.addListener(window, "touchstart", onTouchStart, listenerOpts);
	...
	var VisibilityMonitor = function(w, t, i) {
		...
		PFLO.subscribe("visibility_changed", function(e) {
			var now = PFLO.now();
			...
			t.log(LOG_TYPE_VIS, now, {
				s: VIS_MAP[PFLO.visibilityState()]
			});
	...
	var OrientationMonitor = function(w, t, i) {
		...
		function onOrientationChange(e) {
			var now = PFLO.now(), angle = window.orientation;
		...
		function stop() {
			...
			PFLO.utils.removeListener(w, "orientationchange", onOrientationChange);
		}

		//
		// Setup
		//
		PFLO.utils.addListener(w, "orientationchange", onOrientationChange, listenerOpts);
	...
	var StatsMonitor = function(w, t) {
		...
		// MutationObserver
		if (PFLO.utils.isMutationObserverSupported()) {
			// setup the observer
			observer = PFLO.utils.addObserver(
				d,
	...
	impl = {
		...
		*/
		...
		runAllAnalyzers: function() {
			...
			impl.timeOfLastBeacon = PFLO.now();
		...
		onXhrLoad: function(data) {
			...
			if (data && PFLO.utils.inArray(data.initiator, PFLO.constants.BEACON_TYPE_SPAS)) {
				impl.isSpa = true;
			}
		...
		onPageReady: function() {
			...
			if (impl.waitAfterOnload) {
				...
				setTimeout(function checkTti() {
					...
					if (PFLO.now() - start > impl.waitAfterOnload) {
	...
	PFLO.plugins.Continuity = {
		 ...
		 * or not to wait for {@link PFLO.plugins.Continuity.frameworkReady} before
		 ...
		 *
		 * @returns {@link PFLO.plugins.Continuity} The Continuity plugin for chaining
		 * @memberof PFLO.plugins.Continuity
		 */
		init: function(config) {
			PFLO.utils.pluginConfig(impl, config, "Continuity",
			...
			// create the timeline
			impl.timeline = new Timeline(PFLO.now());
			...
			if (PFLO.window) {
				...
				if (impl.monitorLongTasks &&
				    PFLO.window.PerformanceObserver &&
				    PFLO.window.PerformanceLongTaskTiming) {
					impl.longTaskMonitor = new LongTaskMonitor(PFLO.window, impl.timeline);
				...
				if (impl.monitorFrameRate &&
				    typeof PFLO.window.requestAnimationFrame === "function") {
					impl.frameRateMonitor = new FrameRateMonitor(PFLO.window, impl.timeline);
				...
				if (impl.monitorPageBusy &&
					(!PFLO.window.PerformanceObserver || !PFLO.window.PerformanceLongTaskTiming || !impl.monitorLongTasks)) {
					impl.pageBusyMonitor = new PageBusyMonitor(PFLO.window, impl.timeline);
				...
				if (impl.monitorInteractions) {
					impl.interactionMonitor = new InteractionMonitor(PFLO.window, impl.timeline, impl.afterOnloadMinWait);
					impl.scrollMonitor = new ScrollMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.keyMonitor = new KeyMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.clickMonitor = new ClickMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.mouseMonitor = new MouseMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.visibilityMonitor = new VisibilityMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.orientationMonitor = new OrientationMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.touchStartMonitor = new TouchStartMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.mouseDownMonitor = new MouseDownMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
					impl.pointerDownMonitor = new PointerDownMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
				}
				...
				if (impl.monitorStats) {
					impl.statsMonitor = new StatsMonitor(PFLO.window, impl.timeline, impl.interactionMonitor);
				}

				if (impl.monitorLayoutShifts &&
					PFLO.window.PerformanceObserver) {
					impl.layoutShiftMonitor = new LayoutShiftMonitor(PFLO.window);

				}
			}

			// add epoch and polling method to every beacon
			PFLO.addVar("c.e", epoch.toString(36));
			PFLO.addVar("c.tti.m", impl.ttiMethod);

			// event handlers
			PFLO.subscribe("before_beacon", impl.onBeforeBeacon, null, impl);
			PFLO.subscribe("beacon", impl.onBeacon, null, impl);
			PFLO.subscribe("page_ready", impl.onPageReady, null, impl);
			PFLO.subscribe("xhr_load", impl.onXhrLoad, null, impl);
